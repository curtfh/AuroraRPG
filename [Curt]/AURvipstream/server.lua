...
function wanttostream(plr)
 if (getElementData(plr, "VIP") == "Yes") then
		triggerClientEvent(plr, "AURvipstream.stream", plr)
	end
end
...
function handleColSpeaker(element)
	...
	if (isElement(usingElement) and getElementType(usingElement) == "vehicle") then
		triggerClientEvent(element, "AURvipstream.startClientStream", element, stream, radius, x, y, z, int, dim, usingElement)
	else
		triggerClientEvent(element, "AURvipstream.startClientStream", element, stream, radius, x, y, z, int, dim)
	end
end
...
addEvent("AURvipstream.streamStart", true)
addEventHandler("AURvipstream.streamStart", root, startStream)
...
function stopStream()
	...
	if (stream) then
		...
		stream = false
		for a,b in pairs(getElementsByType("player")) do
			triggerClientEvent(b, "AURvipstream.streamClientStop", b)
		end
		return true
	end
end
addEvent("AURvipstream.streamStop", true)
addEventHandler("AURvipstream.streamStop", root, stopStream)