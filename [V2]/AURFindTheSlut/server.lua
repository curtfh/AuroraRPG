...
function start()
    ...
    exports.NGCdxmsg:createNewDxMessage("A slut is asking for the D. She is stripping on top of her car at "..location..". Go finger her for some extra health.",root, 0, 255, 0)
    timerbeforeEnd = setTimer(Over, 7 * 40 * 1000, 1)
end
...
function Over(p)
    ...
    if (p)then
		if (chance >= 50) then
			...
			exports.NGCdxmsg:createNewDxMessage(getPlayerName(p).." found the slut!",root, 0, 255, 0)
			killTimer(timerbeforeEnd)
		else
			exports.NGCdxmsg:createNewDxMessage(getPlayerName(p).." found the slut!",root, 0, 255, 0)
			exports.NGCdxmsg:createNewDxMessage("But she sadly has aids so he died.",root, 0, 255, 0)
			killPed(p)
			killTimer(timerbeforeEnd)
		end
    else
        if (slut)then
            if (isPedDead(slut))then
                exports.NGCdxmsg:createNewDxMessage("The slut died :(",root, 0, 255, 0)
                killTimer(timerbeforeEnd)
            else
                exports.NGCdxmsg:createNewDxMessage("The slut couldn't find anyone. She ended up using a dildo...",root, 0, 255, 0)
            end
        end
    end
    ...
    GlobalTimer = setTimer(start, 10 * 40 * 1000, 1)
end

addEventHandler("onResourceStart", root,
    function (res) -- init job if thisResource started
