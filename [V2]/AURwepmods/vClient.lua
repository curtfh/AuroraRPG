...
addEventHandler("onClientResourceStart", resourceRoot,
    function()
        panel = guiCreateWindow(229, 146, 763, 542, "AuroraRPG  - Weapon Mods Panel", false)
...
---- Information Label - Big Text
	...
	searchEd = guiCreateEdit(644, 76, 104, 23, "", false, panel)
        ...
        searchLab = guiCreateLabel(644, 56, 103, 20, "Weapon Name:", false, panel)
        guiSetFont(searchLab, "default-bold-small")
        guiLabelSetColor(searchLab, 225, 225, 225)
...
addEvent ("userpanel_weaponmods", true )
addEventHandler ("userpanel_weaponmods", root, vehModsCmd )
addCommandHandler("weaponmods", vehModsCmd )
...
function()
	...
	guiBringToFront(searchEd)
        ...
        --else
            --exports.NGCdxmsg:createNewDxMessage("Leave your vehicle first!", 255, 0, 0 )
        --end
	...
	guiBringToFront(searchEd)
        if ( iDownload == "Yes" ) then
            if ( iEnable == "Yes" ) then  
                exports.NGCdxmsg:createNewDxMessage("Disable this mod first", 255, 0, 0) 
            return end
...
function deleteMod(name)
	local _, _, dff, txd = getModData(name)
        if ( fileExists("mods/"..txd) ) then
            fileDelete("mods/"..txd)
            exports.NGCdxmsg:createNewDxMessage('TXD file has been deleted successfully!', 0, 255, 0)
            refreshList()
        end 
        
        if ( fileExists("mods/"..dff) ) then
            fileDelete("mods/"..dff)
            exports.NGCdxmsg:createNewDxMessage('DFF file has been deleted successfully!', 0, 255, 0)
            refreshList()
        end
end
                ...
                if ( fileExists("mods/"..txd) ) then
                    if ( state == false ) then
                        ...
                        if ( weirdbool ) then
                            ...
                            exports.NGCdxmsg:createNewDxMessage("Another mod already enabled at the same weapon.", 255, 0, 0 )
                            return end
                            local txd1 = engineLoadTXD ( "mods/"..txd )
                            if ( not txd1 ) then
                                exports.NGCdxmsg:createNewDxMessage("Failed to load the mod", 255, 0, 0 )
                            end
                        ...
                        if ( not engineReplaceModel ( dff , id ) ) then
                            exports.NGCdxmsg:createNewDxMessage("Failed to replace the mod", 255, 0, 0)
                        end
                        eTable[name] = true
                        enabledIDs[id] = true
...
function downloadMod(name)
	...
	refreshList()
	dTable[name] = true
	exports.NGCdxmsg:createNewDxMessage('Please wait while downloading your mod!', 0, 255, 0)
end
...
function onDownloadFinish (file, success)
    if ( source == resourceRoot ) then
        if ( success ) then
            refreshList()
            exports.NGCdxmsg:createNewDxMessage("Your mod has been successfully downloaded ("..tostring(file)..")", 0, 255, 0)
        end
    end
end
