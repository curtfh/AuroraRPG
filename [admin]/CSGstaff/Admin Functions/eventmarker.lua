		...
		if ( isPlayerStaff( thePlayer )) then
			...
			if (data.eventmanager == 1) then
				if ( string.lower( cmd ) == "create" ) then
					...
					exports.NGCdxmsg:createNewDxMessage("A event has been placed by "..getPlayerName(thePlayer)..", head to the blip!",root,0,255,0)
...
addEventHandler("onPlayerQuit",root,function()
	if eventMarkers[source] and isElement(eventMarkers[source]) then
		destroyElement(eventMarkers[source])
		destroyElement( eventMarkerBlips[source] )
		eventMarkers[source] = {}
		eventMarkerBlips[source] = {}
	end
end)
