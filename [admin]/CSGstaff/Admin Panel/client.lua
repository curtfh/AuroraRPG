...
--[[local punishmentReaons = {
	"#00 - Custom Reason",
	"#01 - Deathmatching",
	"#02 - Testing Punishments",
	"#03 - Disrespecting Staff Members",
	"#04 - Exploiting/abusing bug(s)",
	"#05 - Cheating/Scaming",
	"#06 - Insulting/Flaming/Provoking",
	"#07 - Advertising for other servers",
	"#08 - Support channel misuse",
	"#09 - Avoiding in-game situations",
	"#19 - Non-English",
	"#11 - Advertising outside advert chat",
	"#12 - Spamming/flooding chats",
	"#13 - Selling in-game contents",
	"#14 - Using Inappropriate name",
	"#15 - Trolling / Griefing",
	"#16 - Heliturfing",
	"#17 - Ruining Server Event",
	"#18 - Requesting Unfair Advantages",
	"#19 - Camping",
	"#20 - Annoying server staff(s)",
	"#21 - Using chats for advertising",
	"#22 - Not listening to staff",
	"#23 - Disrespecting players",
	"#24 - Using illegal programs",
	"#25 - Sharing illegal programs",
	"#26 - Impersonating another player",
	"#27 - Talking about religion/sexual interests",
	"#28 - Multiaccounting for abusing",
	"#29 - Speaking about other servers",
	"#30 - Blackmailing",
	"#31 - Misusing reporting system",
}]]--
...
local punishmentReaons = {
	...
	"#01 - Deathmatching",
	"#02 - Annoying server staff(s)",
	"#02 - Disrespecting Staff Members/Players",
	"#03 - Exploiting/Abusing Bug(s)/Cheating",
	"#04 - Insulting/Flaming/Provoking",
	"#04 - Racism",
	"#04 - Causing Bad Atmosphere",
	"#05 - Misusing Armed Vehicles",
	"#06 - Advertising",
	"#07 - Support channel misuse",
	"#08 - Avoiding in-game situations",
	"#09 - Non-English",
	"#10 - Misusing Advertising Chat",
	"#11 - Spamming/flooding chats",
	"#11 - Talking about religion/sexual interests",
	"#12 - Selling in-game contents",
	"#12 - Abusing through accounts",
	"#13 - Trolling / Griefing",
	"#14 - Heliturfing",
	"#15 - Camping",
	"#16 - Blackmailing",
	"#17 - Using illegal programs",
	"#17 - Sharing illegal programs",
	"#18 - Using Inappropriate name",
	"#18 - Impersonating another player",
	"#17 - Ruining Server Event",
	"#18 - Requesting Unfair Advantages",
	"#19 - Advertising outside advert chat",
	"#19 - Misuse of reporting system",
	"#20 - Bad Behaviour",
	"#21 - Using chats for advertising",
	"#22 - Not listening to staff",
	"#23 - Disrespecting players",
...
-- The window
adminGUI.GUIwindow[1] = guiCreateWindow(348, 156, 862, 610, "AuroraRPG ~ Admin Panel", false)
...
adminGUI.GUIlabels[26] = guiCreateLabel(10, 388, 298, 18, "VIP Hours: N/A", false, adminGUI.GUItabs[1])
...
adminGUI.GUIchecks[2] = guiCreateCheckBox(327, 140, 130, 15, "VFC?", false, false, adminGUI.GUItabs[2])
adminGUI.GUIchecks[7] = guiCreateCheckBox(327, 120, 130, 15, "Use Multiplier", false, false, adminGUI.GUItabs[2])
...
adminGUI.GUIbuttons[4] = guiCreateButton(491, 209, 154, 25, "Reconnect", false, adminGUI.GUItabs[2])
...
adminGUI.GUIbuttons[15] = guiCreateButton(491, 279, 154, 25, "Give VIP Car", false, adminGUI.GUItabs[2])
...
adminGUI.GUIchecks[8] = guiCreateCheckBox(491, 125, 154, 25, "Hide Name", false, false, adminGUI.GUItabs[2])
...
adminGUI.GUIlabels[41] = guiCreateLabel(439, 91, 170, 17, "VIP hours:", false, adminGUI.GUItabs[4])
...
guiCreateLabel(443, 244, 172, 15, "PIN:", false, adminGUI.GUItabs[4])
adminGUI.accountsPINCodeEdit = guiCreateEdit(439, 261, 201, 24, "", false, adminGUI.GUItabs[4])
...
adminGUI.GUIlabels[59] = guiCreateLabel(10, 4, 334, 16, "All Aurora Admins:", false, adminGUI.GUItabs[6])
...
adminGUI.GUIbuttons[800] = guiCreateButton(476, 519, 174, 24, "Join Group as a Leader", false, adminGUI.GUItabs[6])
...
adminGUI.globalBanCheckbox = guiCreateCheckBox(275, 499, 290, 16, "Global ban ( ban will count for all Aurora servers )", false,false, adminGUI.GUItabs[7])
...
adminGUI.GUItabs[8] = guiCreateTab("Rooms", adminGUI.GUItabbar[1])
adminGUI.GUIgrids[90] = guiCreateGridList(2, 3, 350, 550, false, adminGUI.GUItabs[8])
adminGUI.GUIbuttons[90] = guiCreateButton(360, 10, 100, 26, "Kick", false, adminGUI.GUItabs[8])
adminGUI.GUIbuttons[91] = guiCreateButton(360, 60, 100, 26, "Unban", false, adminGUI.GUItabs[8])
adminGUI.GUIbuttons[92] = guiCreateButton(530, 10, 100, 26, "Ban", false, adminGUI.GUItabs[8])
...
guiGridListAddColumn( adminGUI.GUIgrids[90], "Players:", 0.5 )
guiGridListAddColumn( adminGUI.GUIgrids[90], "Room:", 0.2 )
guiGridListAddColumn( adminGUI.GUIgrids[90], "Alive:", 0.2 )
...
function getRoomSelectedPlayer ()
	local playerName = guiGridListGetItemText ( adminGUI.GUIgrids[90], guiGridListGetSelectedItem ( adminGUI.GUIgrids[90] ), 1 )
	local row, column = guiGridListGetSelectedItem ( adminGUI.GUIgrids[90] )
...
function loadRooms()
	guiGridListClear( adminGUI.GUIgrids[90] )
	for k, thePlayer in ipairs ( getElementsByType( "player" ) ) do
		if getElementDimension(thePlayer) >= 5001 and getElementDimension(thePlayer) <= 5004 then
			local row = guiGridListAddRow ( adminGUI.GUIgrids[90] )
			if ( getPlayerTeam( thePlayer ) ) then
				...
				guiGridListSetItemText ( adminGUI.GUIgrids[90], row, 1, getPlayerName( thePlayer ), false, true )
				guiGridListSetItemText ( adminGUI.GUIgrids[90], row, 2, theRoom, false, true )
				if veh and isElement(veh) then
					guiGridListSetItemText ( adminGUI.GUIgrids[90], row, 3,"Yes", false, true )
				else
					guiGridListSetItemText ( adminGUI.GUIgrids[90], row, 3,"No", false, true )
				end
				guiGridListSetItemColor( adminGUI.GUIgrids[90], row, 1, rx,gx,bx )
			else
				guiGridListSetItemText ( adminGUI.GUIgrids[90], row, 1, getPlayerName( thePlayer ), false, true )
			end
		end
...
addEventHandler( "onClientRender", root,
	function ()
		...
		if ( getAdminPanelSelectedPlayer () ) then
			...
			local country = getElementData(thePlayer,"Country") or "Unknown"
			...
			guiSetText(adminGUI.GUIlabels[7], "Country: "..country)
		end
	end
)
...
addEventHandler( "onRequestAdminPlayerInfo:callBack", root,
	function ( theTable )
		...
		guiSetText( adminGUI.GUIlabels[26], "VIP Hours: "..theTable[3] or "N/A" )
...
addEvent( "onRequestAccountPIN:callBack",true)
addEventHandler( "onRequestAccountPIN:callBack",root,
	function (accountID,PIN)
		accountPIN[tonumber(accountID)] = tonumber(PIN)
		if accountID == selectedAccountID then
			guiSetText(adminGUI.accountsPINCodeEdit,PIN or "Error")
		end
	end
)
...
function onAccountGridClick()
	...
	if selRow and selRow ~= -1 then
		...
		if type(accInfo) == 'table' then
			...
			VIPHours = math.ceil(accInfo['VIP']/60)
			...
			--else
				...
				triggerServerEvent('onRequestAccountPIN',localPlayer,accountID)
			--end
			selectedAccountID = accountID
		end
	end
	if not APS then APS = 0 end
	if APS and APS == "N/A" then APS = 0 end
	if APS and tonumber(APS) < 0 then APS = 0 end
	guiSetText(adminGUI.GUIedits[8],username)
	guiSetText(adminGUI.GUIedits[9],playtime)
	guiSetText(adminGUI.GUIedits[10],VIPHours)
	...
	guiSetText(adminGUI.accountsPINCodeEdit,PIN)
...
function deleteSelectedAccount()
	...
	if selRow and selRow ~= -1 then
		...
		exports.NGCdxmsg:createNewDxMessage('No account selected!',255,0,0)
	end
end
...
function getUpdatedAccountValues(rowData)
	...
	if selRowData['username'] ~= guiGetText(adminGUI.GUIedits[8]) then
		if not string.match(guiGetText(adminGUI.GUIedits[8]),"^%s*$") then
			...
			exports.NGCdxmsg:createNewDxMessage("Username invalid!",255,0,0)
			return false;
		end
	end
	if math.ceil(selRowData['playtime']/60) ~= tonumber(guiGetText(adminGUI.GUIedits[9])) then
		if tonumber(guiGetText(adminGUI.GUIedits[9])) and tonumber(guiGetText(adminGUI.GUIedits[9])) >= 0 then
			...
			exports.NGCdxmsg:createNewDxMessage("Playtime is not a number, or below 0.",255,0,0)
			return false;
		end
	end
	if math.ceil(selRowData['VIP']/60) ~= tonumber(guiGetText(adminGUI.GUIedits[10])) then
		if tonumber(guiGetText(adminGUI.GUIedits[10])) and tonumber(guiGetText(adminGUI.GUIedits[10])) >= 0 then
			updatedValuesTable["VIP"] = tonumber(guiGetText(adminGUI.GUIedits[10]))*60
		else
			exports.NGCdxmsg:createNewDxMessage("VIP hours is not a number, or below 0.",255,0,0)
			return false;
		end
	end
	...
	if selRowData['money'] ~= tonumber(guiGetText(adminGUI.GUIedits[14])) then
		if tonumber(guiGetText(adminGUI.GUIedits[14])) then
			...
			exports.NGCdxmsg:createNewDxMessage("Money is not a number.",255,0,0)
			return false;
		end
	end
	if not accountBalances[selRowData['id']] or accountBalances[selRowData['id']] ~= tonumber(guiGetText(adminGUI.GUIedits[15])) then
		if tonumber(guiGetText(adminGUI.GUIedits[15])) then
			...
			exports.NGCdxmsg:createNewDxMessage("Bank money is not a number.",255,0,0)
			return false;
		end
	end
	if not accountPIN[selRowData['id']] or accountPIN[selRowData['id']] ~= tonumber(guiGetText(adminGUI.accountsPINCodeEdit)) then
		if tonumber(guiGetText(adminGUI.accountsPINCodeEdit)) then
			updatedValuesTable["PIN"] = tonumber(guiGetText(adminGUI.accountsPINCodeEdit))
		else
			exports.NGCdxmsg:createNewDxMessage("PIN is not a number.",255,0,0)
			return false;
		end
	end
	if selRowData['score'] ~= tonumber(guiGetText(adminGUI.accountsScoreEdit)) then
		if tonumber(guiGetText(adminGUI.accountsScoreEdit)) then
			...
			exports.NGCdxmsg:createNewDxMessage("Score is not a number.",255,0,0)
			return false;
		end
	end
	--[[if selRowData['PIN'] ~= tonumber(guiGetText(adminGUI.accountsPINCodeEdit)) then
		if tonumber(guiGetText(adminGUI.accountsPINCodeEdit)) then
			updatedValuesTable["PIN"] = tonumber(guiGetText(adminGUI.accountsPINCodeEdit))
		else
			exports.NGCdxmsg:createNewDxMessage("PIN is not a number.",255,0,0)
	...
	if #guiGetText(adminGUI.GUIedits[16]) > 0 then
		if #(string.gsub(guiGetText(adminGUI.GUIedits[16])," ","")) >= 4 then
			if guiGetText(adminGUI.GUIedits[16]) == guiGetText(adminGUI.GUIedits[17]) then
				...
				exports.NGCdxmsg:createNewDxMessage("Passwords must match.",255,0,0)
				return false;
			end
		else
			exports.NGCdxmsg:createNewDxMessage("Password must be 4 characters long.",255,0,0)
			return false;
		end
	end
...
addEventHandler("onClientGUIDoubleClick", adminGUI.GUIgrids[2],function()
	...
	if getPlayerAdminLevel(localPlayer) >= 3 then
...
addEventHandler ( "onClientGUIClick", root,
	function ()
		...
		elseif ( source == adminGUI.GUIbuttons[53] ) then -- Join leader
			local username = guiGetText(adminGUI.GUIedits[40])
			local nickname = guiGetText(adminGUI.GUIedits[41])
			if username and nickname then
				if guiRadioButtonGetSelected(adminGUI.GUIradios[10]) then
					gender = 1
				elseif guiRadioButtonGetSelected(adminGUI.GUIradios[9]) then
					gender = 0
				end
				if gender == 0 or gender == 1 then
					triggerServerEvent("addServerStaff",localPlayer,username,nickname,gender)
				else
					exports.NGCdxmsg:createNewDxMessage("Please select gender",255,0,0)
				end
			else
				exports.NGCdxmsg:createNewDxMessage("Please insert nickname and username!",255,0,0)
			end
		end
	end
)
...
addEventHandler ( "onClientGUIClick", root,
	function ()
		...
		elseif ( source == adminGUI.GUIbuttons[4]  ) then -- Reconnect Player
			triggerServerEvent( "onAdminPlayerActions", localPlayer, thePlayer, "reconnect" )
		...
		elseif ( source == adminGUI.GUIbuttons[10] ) then -- Give Vehicle
			if ( getVehicleModelFromName ( guiGetText( adminGUI.GUIedits[4] ) ) or getVehicleNameFromModel ( tonumber(guiGetText( adminGUI.GUIedits[4] ) )) ) then
				...
				exports.NGCdxmsg:createNewDxMessage ( "No vehicle found with that name!", 0, 225, 0 )
			end
		...
		elseif ( source == adminGUI.GUIbuttons[15] ) then -- Give VIP Car
			triggerServerEvent( "onAdminPlayerActions", localPlayer, thePlayer, "VIP" )
		elseif ( source == adminGUI.GUIbuttons[16] ) then -- Give Weapon
			if ( getWeaponIDFromName ( guiGetText( adminGUI.GUIedits[5] ) ) or ( tonumber(guiGetText( adminGUI.GUIedits[5] )) and getWeaponNameFromID(tonumber(guiGetText( adminGUI.GUIedits[5] )))) ) and ( string.match( guiGetText( adminGUI.GUIedits[6] ), '^%d+$' ) ) then triggerServerEvent( "onAdminPlayerActions", localPlayer, thePlayer, "weapon", getWeaponIDFromName ( guiGetText( adminGUI.GUIedits[5] ) ) or tonumber(guiGetText( adminGUI.GUIedits[5] )), tonumber( guiGetText( adminGUI.GUIedits[6] ) ) ) else exports.NGCdxmsg:createNewDxMessage ( "No weapon found with that name/ID!", 0, 225, 0 ) end
...
function onBanClick()
	if ( source == adminGUI.GUIbuttons[54] or source == adminGUI.GUIbuttons[58] ) then -- ban account/serial
		...
		if ( banInput and string.match( banInput, '^[%w%s]*%w[%w%s]*$' ) and banInput ~= "Serial/Accountname" ) and ( reason and string.match( reason, '^[%w%s]*%w[%w%s]*$' ) and reason ~= "Reason" ) then
			...
			if banTime and type(banTime) == 'number' then
				...
				exports.NGCdxmsg:createNewDxMessage("Please give a valid ban time ( number )",255,0,0)
			end
		else
			exports.NGCdxmsg:createNewDxMessage("Please give a valid serial/account and reason.",255,0,0)
		end
	elseif ( source == adminGUI.GUIbuttons[59] ) then -- unban selected
		...
		if banInfo then
			triggerServerEvent( "staffpanel.unban", localPlayer, banInfo.id, banInfo.global, localPlayer)
...
function onRoomClick()
	if source == adminGUI.GUIbuttons[90] then
		local thePlayer = getRoomSelectedPlayer()
		if thePlayer then
			if getElementDimension(thePlayer) == 5001 then
			...
			elseif getElementDimension(thePlayer) == 5003 then
				...
				exports.NGCdxmsg:createNewDxMessage("Current player is not in any room",255,0,0)
			end
		else
			exports.NGCdxmsg:createNewDxMessage("Current player is offline",255,0,0)
		end
	end
end
addEventHandler('onClientGUIClick',adminGUI.GUIbuttons[90],onRoomClick,false)
...
function getPunishmentTime ()
	...
	elseif ( string.match( guiGetText( adminGUI.GUIedits[3] ), '^%d+$' ) ) then
		...
		exports.NGCdxmsg:createNewDxMessage ( "No valid punishment time enterd!", 0, 225, 0 )
		return false
	end
end
	...
	function ()
		...
		if ( guiCheckBoxGetSelected ( adminGUI.GUIchecks[1] ) ) then
			if ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Mainchat/teamchat mute" )  then
				triggerServerEvent( "onServerPlayerPunishRemove", localPlayer, thePlayer, "Mainchat/teamchat mute", guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]) )
			elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Global mute" ) then
				triggerServerEvent( "onServerPlayerPunishRemove", localPlayer, thePlayer, "Global mute", guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]) )
			elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Jail" ) then
				triggerServerEvent( "onServerPlayerPunishRemove", localPlayer, thePlayer, "Jail", guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]) )
			end
		elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Mainchat/teamchat mute" ) and ( getPunishmentTime () ) then
			if ( getPunishmentTime () == 0 ) then exports.NGCdxmsg:createNewDxMessage ( "You can't mute someone permanently!", 0, 225, 0 ) return end
			triggerServerEvent( "onServerPlayerPunish", localPlayer, thePlayer, "Mainchat/teamchat mute", getPunishmentTime (), getPunishmentReason (), guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]))
		elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Global mute" ) and ( getPunishmentTime () ) then
			if ( getPunishmentTime () == 0 ) then exports.NGCdxmsg:createNewDxMessage ( "You can't mute someone permanently!", 0, 225, 0 ) return end
			triggerServerEvent( "onServerPlayerPunish", localPlayer, thePlayer, "Global mute", getPunishmentTime (), getPunishmentReason (), guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]) )
		elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Jail" ) and ( getPunishmentTime () ) then
			if ( getPunishmentTime () == 0 ) then exports.NGCdxmsg:createNewDxMessage ( "You can't jail someone permanently!", 0, 225, 0 ) return end
			triggerServerEvent( "onServerPlayerPunish", localPlayer, thePlayer, "Jail", getPunishmentTime (), getPunishmentReason (), guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]) )
		elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Account ban" ) and ( getPunishmentTime () ) then
			if (guiCheckBoxGetSelected(adminGUI.GUIchecks[7])) then
				outputChatBox("Multiplier does not work with bans", 255, 0, 0)
				return false
			end
			triggerServerEvent( "onServerPlayerPunish", localPlayer, thePlayer, "Account ban", getPunishmentTime (), getPunishmentReason (), guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]) )
		elseif ( guiComboBoxGetItemText ( adminGUI.GUIcombos[2], guiComboBoxGetSelected ( adminGUI.GUIcombos[2] ) ) == "Serial ban" ) and ( getPunishmentTime () ) then
			...
			triggerServerEvent( "onServerPlayerPunish", localPlayer, thePlayer, "Serial ban", getPunishmentTime (), getPunishmentReason (), guiCheckBoxGetSelected(adminGUI.GUIchecks[2]), guiCheckBoxGetSelected(adminGUI.GUIchecks[7]), guiCheckBoxGetSelected(adminGUI.GUIchecks[8]))
		end
	end, false
)
...
function(button,state)
	if (source == adminGUI.GUIbuttons[22]) then --create event
		...
		if not (eventName ~= "") and not (maxPlayers ~= "") and not (type(maxPlayers) == "string") then
			...
			exports.NGCdxmsg:createNewDxMessage(localPlayer,"Eventname or maxplayers is not filled correctly.",255,0,0)
		end
