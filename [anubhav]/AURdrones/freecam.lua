...
local function freecamFrame ()
   ...
   -- camPosZ = camPosZ + normalZ * strafespeed
    ...
    local z = getGroundPosition(camPosX, camPosY, camPosZ)
    if (camPosZ - z <= 5) then
    	maxSpeedKey = true
    	if (camPosZ - z <= 1) then
    		setFreecamDisabled()
    		fireMissiles(camPosX, camPosY, camPosZ)
    		return true
    	end
    end
    if (getControlState("fire") or getControlState("vehicle_fire")) then
    	maxSpeedKey = true
    end
    ...
    setCameraMatrix ( camPosX, camPosY, camPosZ - removeZ, camTargetX, camTargetY, camTargetZ )
end
