...
EMgridControls = {
	...
	[12] = {{"   Destroy Spawner/Cars", "Destroy"}, function() triggerServerEvent("AURevents.DelVehStuff", localPlayer) end, 4},
	...
	[16] = {{"   Disable 'Blips'", "Disable"}, {"   Enable 'Blips'", "Enable"}, "ToggleBlip", 1},
	[17] = {{"   Warp event players to me", "Warp"}, "WarpEventPlayersToEM", 1},
	[18] = {{"   Disable vehicle leaving", "Disable"}, {"   Enable vehicle leaving", "Enable"}, "ToggleVehicleLeaving", 2},
	[19] = {{"   Disable collisions", "Disable Collisions"}, {"   Enable collisions", "Enable Collisions"}, "ToggleCollisions", 2},
	[20] = {{"   Set everyone off-duty", "Set"}, {"   Set everyone on-duty", "Set"}, "TogglePlayerWorking", 1},
	[21] = {{"   Load/Save Event", "Open Window"}, function() triggerServerEvent("AURevents.GetEvents", root) end, 4},
	[22] = {{"   Disable Weapons", "Disable Weapons"}, {"   Enable Weapons", "Enable Weapons"}, "ToggleWeapons", 1},
	[23] = {{"   Disable hijacking", "Disable"}, {"   Enable hijacking", "Enable"}, "ToggleVehicleHijacking", 2},
	[24] = {{"   Disable Team Killing", "Disable Team Killing"}, {"   Enable Team Killing", "Enable Team Killing"}, "ToggleTeamKilling", 1},
	[25] = {{"   Change Weather", "Change Weather"}, "ChangeWeather", 3, {{9999999, "Weather ID (http://wiki.multitheftauto.com/wiki/Weather)", 0}}},
	[26] = {{"   Change Wave Height", "Change Wave Height"}, "ChangeWaveHeight", 3, {{100, "0-100", 0}}},
	[27] = {{"   Change Water Level", "Change Water Level"}, "ChangeWaterlevel", 3, {{100, "-100-100", -100}}},
	[28] = {{"   Element Explorer", "Open Window"}, function() triggerServerEvent("AURevents.GetElements", root) end, 4},
}
...
addEvent("AURevents.RefreshCData", true)
addEventHandler("AURevents.RefreshCData", root, processList)
...
function onActionClick()
	...
	if (index) then
		...
		if (type(func) == "string") then
			triggerServerEvent("AURevents.Action", root, func, index, guiGetText(EMedits[6]), guiGetText(EMedits[7]))
...
function startEvent()
	...
	if (limit ~= "" and getTeamName(getPlayerTeam(localPlayer)) == "Staff") then
		local teamJobs
		local forbidTeams = {}
		local allowedTeams = {}
		local count = 0
		...
		if (guiCheckBoxGetSelected(EMcheckboxes[4])) then
			...
			if (count == 0) or (count == #teamJobs) then
				...
				exports.NGCdxmsg:createNewDxMessage("You can forbid/only allow job/teams at the same time", 255, 0, 0)
				return
			end
		end	
		if (guiGetText(source) == "Start Event") then
			triggerServerEvent("AURevents.PreCreateEvent", root, limit, guiCheckBoxGetSelected(EMcheckboxes[3]), guiCheckBoxGetSelected(EMcheckboxes[1]), guiCheckBoxGetSelected(EMcheckboxes[2]), tableToSend, forbid)
			hideEMwindow()
		else
			if (tonumber(guiGetText(EMedits[1]))) then
				triggerServerEvent("AURevents.AddWarps", root, limit, guiCheckBoxGetSelected(EMcheckboxes[3]), guiCheckBoxGetSelected(EMcheckboxes[2]), tableToSend, forbid)
				hideEMwindow()
			end
		end
	end
...
function stopEvent()
	if (limit ~= "" and getTeamName(getPlayerTeam(localPlayer)) == "Staff") then
		triggerServerEvent("AURevents.StopEvent", root)
		hideEMwindow()
	end
end
...
addEvent("AURevents.ToggleClientFeature", true)
addEventHandler("AURevents.ToggleClientFeature", root, toggleClientFeature)
...
function applyEventDimFeatures(config)
	if (config and getElementDimension(localPlayer) == 336) then
		...
		if (config[3]) then
			setWaterLevel(config[3])
		end
	end
end
addEvent("AURevents.SendEventDimFeature", true)
addEventHandler("AURevents.SendEventDimFeature", root, applyEventDimFeatures)
...
addEvent("AURevents.RemoveEventDimFeatures", true)
addEventHandler("AURevents.RemoveEventDimFeatures", root, removeEventDimFeatures)

function EDFCheck()
	local dim = getElementDimension(localPlayer)
	if (dim == 336) then
		if (not eventDimFeatures) then
			triggerServerEvent("AURevents.GetEventDimFeatures", root)
		end
	else
		if (eventDimFeatures) then
			triggerServerEvent("AURevents.GetNormalFeaturesState", root)
		end
	end
end
...
function onPlayerAction()
	...
	if (currentAction > 0 and #players > 0) then
		...
		if (type(action) == "string") then
			triggerServerEvent("AURevents.SingleAction", root, action, tableOfPlayers, text1, text2)
