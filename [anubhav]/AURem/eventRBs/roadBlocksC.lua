...
function number_mode_function(elementChanged)
	...
	if (tonumber(newText) < 0) then
		...
		return true
	elseif (tonumber(newText) > 255) then
...
function createRoadblockGUI()
	...
	guiLabelSetColor(guiLabel, 0, 255, 0)
	...
	number_mode_function(redEdit, true, true, true)
	number_mode_function(greenEdit, true, true, true)
	number_mode_function(blueEdit, true, true, true)
	
	addEventHandler("onClientGUIClick", destroyButton, 
		function() 
			triggerEvent("AURevents.DestroyMyRBsCallback", localPlayer) 
		end
	)
	addEventHandler("onClientGUIClick", destroyallButton, 
		function() 
			triggerEvent("AURevents.DestroyALLRBsCallback", localPlayer) 
		end
	)
...
function destroyMyRBs()
	triggerServerEvent("AURevents.RB.DestroyRoadblocks", localPlayer)
end
addEvent("AURevents.DestroyMyRBsCallback")
addEventHandler("AURevents.DestroyMyRBsCallback", root, destroyMyRBs)

function destroyAllRBs()
	triggerServerEvent("AURevents.RB.DestroyALLRoadblocks", localPlayer)
end
addEvent("AURevents.DestroyALLRBsCallback")
addEventHandler("AURevents.DestroyALLRBsCallback", root, destroyAllRBs)
...
function roadblockGUI()
	triggerServerEvent("AURevents.RB.CheckAdminLevel", localPlayer)
end
...
function placeRoadblock()
	...
	if (getElementType(object) == "object") then
		triggerServerEvent("AURevents.RB.AddRoadblock", localPlayer, id, x, y, z, rx, ry, rz, dim, int)
	elseif (getElementType(object) == "vehicle") then
		local r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4 = getVehicleColor(object, true)
		triggerServerEvent("AURevents.RB.SpawnVeh", localPlayer, id, x, y, z, rx, ry, rz, dim, int, {r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4})
	end
	destroyElement(object)	
    unbindKey("mouse2", "down", cancelRoadBlock)
    unbindKey("mouse1", "down", placeRoadblock)
				...
				if (clickedWorld and isElement(clickedWorld)) then
					if (getElementType(clickedWorld) == "object") then
						for k,v in ipairs(getElementsByType("object"), resourceRoot) do
							if (v == clickedWorld) then
								triggerServerEvent("AURevents.RB.DestroyRoadblock", root, clickedWorld)
								return
							end
						end
					elseif (getElementType(clickedWorld) == "vehicle") then
						for k,v in ipairs(getElementsByType("vehicle"), resourceRoot) do
							if (v == clickedWorld) then
								triggerServerEvent("AURevents.RB.DestroyVeh", root, clickedWorld)
								return
							end
						end
					end
