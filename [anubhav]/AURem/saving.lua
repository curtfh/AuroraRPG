...
function warpTo()
	...
	if (name ~= "") then
		triggerServerEvent("AURevents.Warp", root, name)
	end
end
...
addEvent("AURevents.SendEvents", true)
addEventHandler("AURevents.SendEvents", root, receiveEvents)

function saveEvent()
	local name = guiGetText(saveStuf[8])
	if (name ~= "") then
		triggerServerEvent("AURevents.SaveEvent", root, name, {guiCheckBoxGetSelected(saveStuf[9]), guiCheckBoxGetSelected(saveStuf[10]), guiCheckBoxGetSelected(saveStuf[11]), guiCheckBoxGetSelected(saveStuf[12])}, guiCheckBoxGetSelected(saveStuf[20]))
	end
end
...
function loadEvent()
	local name = guiGridListGetItemText(saveStuf[18], guiGridListGetSelectedItem(saveStuf[18]), 2)
	if (name ~= "") then
		triggerServerEvent("AURevents.LoadEvent", root, name)
		guiSetVisible(saveStuf[1], false)
...
function preDelete()
	local name = guiGridListGetItemText(saveStuf[18], guiGridListGetSelectedItem(saveStuf[18]), 2)
	if (name ~= "") then
		triggerEvent("AURevents.delevcb",localPlayer)
	end
end
...
function deleteEvent()
	local name = guiGridListGetItemText(saveStuf[18], guiGridListGetSelectedItem(saveStuf[18]), 2)
	if (name ~= "") then
		triggerServerEvent("AURevents.DeleteEvent", root, name)
	end	
end
addEvent("AURevents.delevcb")
addEventHandler("AURevents.delevcb", root, deleteEvent)
