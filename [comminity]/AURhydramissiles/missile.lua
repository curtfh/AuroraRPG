...
local SHOOT_COOLDOWN = 3000 --Cooldown between homing shots
...
local VALID_TARGET_FUNCTION = function (vehicle) --Used to decide whether a vehicle should appear as a lock-on option
	...
	if (getElementData(localPlayer, "isPlayerInLvCol")) then return false end
	if targetTeam and ourTeam and targetTeam == ourTeam and getTeamName(targetTeam) == "Military Forces" and getTeamName(ourTeam) == "Military Forces" then
...
local function homingState(key,state)
	...
	if state == "down" then
		...
		triggerServerEvent ( "AURhydramissles.triggz", resourceRoot, localPlayer,  "vehicle_secondary_fire",false)
		...
		triggerServerEvent ( "AURhydramissles.triggz", resourceRoot, localPlayer,  "vehicle_secondary_fire",firestate)
		firestate = nil
...
local function startHydra(vehicle)
	if not inHydra and vehicle and isElement(vehicle) and vehicle.model == 520 then
		if (getElementData(localPlayer, "isPlayerInLvCol")) then  
			exports.AURstickynote:displayText("hydranote", "text", "Hydra Missiles: Disabled", 128, 70, 164)
		else
			exports.AURstickynote:displayText("hydranote", "text", "Press H or caps lock to change target. Press Space + Shoot to shoot the target. Note: Red color target means target selected. Yellow color target means stand by target. Green target means friendly target.", 128, 70, 164)
		end 
		...
		triggerServerEvent ( "AURhydramissles.triggz", resourceRoot, localPlayer,  "handbrake",false)
...
stopHydra = function()
	if inHydra then
		exports.AURstickynote:displayText("hydranote", "text", "")
		local target = getTarget()
		for i, v in ipairs(nearbyVehicles) do
			if v ~= target then
		...
		triggerServerEvent ( "AURhydramissles.triggz", resourceRoot, localPlayer,  "handbrake",inHydra=="true")
		inHydra = false
