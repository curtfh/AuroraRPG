...
local LABEL_FUNCTION = function(vehicle)
	if (getElementData(localPlayer, "isPlayerInLvCol")) then return "Error: Detection not available" end
	return getVehicleName(vehicle)
end --Should take a vehicle and return the text to be shown for it (e.g. Player's clan, or vehicle model, or score etc.)
local COLOR_FUNCTION = function(vehicle)
	if (getElementData(localPlayer, "isPlayerInLvCol")) then return 255, 0, 0 end
...
end  --Should take a vehicle (may be empty vehicle) and return R,G,B of colour of the box around it (e.g. team color, or just red and green for "enemy and ally")
--examples for both of these are the default functions below
local VALID_VEHICLE_FUNCTION = function()
	if (getElementData(localPlayer, "isPlayerInLvCol")) then return false end
	return true
end  --Similar to VALID_TARGET_FUNCTION in missile.lua
