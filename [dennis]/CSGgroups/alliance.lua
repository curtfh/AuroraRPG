...
function createNewAlliance()
	if getElementData(localPlayer,"Group") == false then
		exports.NGCdxmsg:createNewDxMessage("Your not in a group!",255,0,0)
		return
	end
	...
	for i=1,#alliances do
		if string.lower(alliances[i].name) == string.lower(allianceName) then
			exports.NGCdxmsg:createNewDxMessage("Alliance name already in use!",255,0,0)
			return false
		end
	end
	if not allianceName:match( '^[%w%s]*%w[%w%s]*$' ) then
		exports.NGCdxmsg:createNewDxMessage("Alliance name invalid!",255,0,0)
		return false
	elseif myAlliance then
		exports.NGCdxmsg:createNewDxMessage("Your group already is an alliance!",255,0,0)
		return false
	elseif getElementData(localPlayer,"GroupRank") == "Group Leader" then
		...
		exports.NGCdxmsg:createNewDxMessage("Your are not Group Leader!",255,0,0)
		return false
	end
end
...
function alliance_confirmAction()
	if isElement(confirmGUI.window) then
		...
		if confirmGUI.authenticate then
			...
			if ( pass:match( "^%s*$" ) ) then exports.NGCdxmsg:createNewDxMessage( "You didn't enter a password!", 225, 0, 0 ) return false end
		end
...
function updateInformation()
	...
	if newText ~= alliances[myAlliance].info then
		if alliance_antiSpam('updateInfo',"updating alliance information") then
			...
			exports.NGCdxmsg:createNewDxMessage("Alliance information succesfully updated!",0,255,0)
		end
	else
		exports.NGCdxmsg:createNewDxMessage("This text is already stored!",255,0,0)
	end
end
getNoteTextGUI = {}
local antiNoteSpam
...
function getNoteText_send()
	if isElement(getNoteTextGUI.window) then
		...
		if ( text:match( "^%s*$" ) ) or ( #text < 1 ) then
			exports.NGCdxmsg:createNewDxMessage("You didn't enter a valid message!", 225, 0, 0 )
...
function getSelectedGroupInMaintenanceTab()
	...
	if ( row ) and ( row > -1 ) then
		...
		exports.NGCdxmsg:createNewDxMessage("You didn't select a group!", 225, 0, 0 )
	end
end
...
function alliance_antiSpam(id,action)
	if isTimer(antiSpamTimers[id]) then
		...
		exports.NGCdxmsg:createNewDxMessage("Please wait "..math.ceil((timeLeft or 1000)/1000).." seconds before "..action.."!", 225, 0, 0 )
		return false
	else
		return true
	end
...
function kickGroup()
	...
	if selected then
		if selected ~= exports.server:getPlayerGroupID(localPlayer) then
			...
			exports.NGCdxmsg:createNewDxMessage("You can't kick your own group, use the leave alliance button instead.", 225, 0, 0 )
		end
	end
end
...
function alliance_getSelectedInvitation()
	...
	if ( row ) and ( row > -1 ) then
		...
		exports.NGCdxmsg:createNewDxMessage("You didn't select an invitation!", 225, 0, 0 )
	end
end
...
function alliance_bankGetAmount()
	...
	if ( not tonumber( theMoney ) ) then
		exports.NGCdxmsg:createNewDxMessage( "You didn't enter a valid amount!", 200, 0, 0 )
	elseif tonumber(theMoney) <= 0 then
		exports.NGCdxmsg:createNewDxMessage( "The amount must be more than $0!", 200, 0, 0 )
