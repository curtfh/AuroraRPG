	...
	function ( thePlayer, theSeat )
		if ( theSeat == 0 ) then
			if ( getElementModel( source ) == 415 )  and ( getPlayerTeam( thePlayer ) ) then
				if exports.DENlaw:isLaw(thePlayer) or ( getTeamName( getPlayerTeam( thePlayer ) ) == "Staff" ) then
					-- Police cheetah
					...
					exports.NGCdxmsg:createNewDxMessage( thePlayer, "This vehicle has custom sirens! Press 'H' to enable them.", 0, 225, 0 )
				end
