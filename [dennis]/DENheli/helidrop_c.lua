...
function AttemptHeliGrab()
	if not getElementData(local_player,"hanging") then
		if target.vehicle and target.distance <= grab_distance then
			...
			if ( getPlayerTeam(local_player) ) and (getElementData(local_player, "Occupation") == "Diver") and (getTeamName(getPlayerTeam(local_player)) == "Civilian Workers") then
				return false
			end
			if getElementData(local_player,"armor",true) then
			return false end
...
function GrabHeli(heli,side,line_percent)
	...
	exports.NGCdxmsg:createNewDxMessage("You can't grab while holding armor",255,0,0)
	return false end
	target.vehicle = heli
	target.side = side
