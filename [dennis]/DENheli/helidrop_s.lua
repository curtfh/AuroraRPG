...
function SetPlayerGrabbedHeli(player,state,heli,side_,line_percent_)
	...
	exports.NGCdxmsg:createNewDxMessage("You can't grab while holding armor",player,255,0,0)
	return false end
	if ( getPlayerTeam(player) ) and (getElementData(player, "Occupation") == "Diver") and (getTeamName(getPlayerTeam(player)) == "Civilian Workers") then
		return false
	end
