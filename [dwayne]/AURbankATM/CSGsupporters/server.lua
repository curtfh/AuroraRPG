...
addEventHandler( "onServerPlayerLogin", root,
	function ()
		...
		if (exports.DENmysql:querySingle("SELECT * FROM supporters WHERE userid=? LIMIT 1",accountID)) then
	...
	function ( thePlayer )
		if ( exports.server:getPlayerAccountName( thePlayer ) ) then
			...
			elseif ( getElementData( thePlayer, "playTime" ) ) and ( getElementData( thePlayer, "playTime" ) < 900 ) then
				onMessageToSupporters ( getPlayerName( thePlayer ).." requested a supporter! Use /warpto [thePlayer] to warp to him/her!" )
...
addCommandHandler( "warpto",
	function ( thePlayer, cmd, aPlayer )
		if getElementData( thePlayer,"isPlayerSupporter" ) == true or exports.CSGstaff:isPlayerStaff(thePlayer) then
			if ( aPlayer ) and not ( aPlayer == "" ) or ( aPlayer == " " ) and ( exports.server:getPlayerFromNamePart( aPlayer ) ) then
				...
				if ( aPlayerElement ) and ( helpTable[ aPlayerElement ] ) then
					if (getElementData(aPlayerElement,"isPlayerJailed") == true) then
