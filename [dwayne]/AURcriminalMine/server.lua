...
function mine (source)
	...
	if isElementWithinColShape(source,LVcol) == false then
		exports.NGCdxmsg:createNewDxMessage(source,"You can only plant a mine in LV!",255,0,0)
		return
	end
	if (getElementData(source, "isPlayerArrested") == true) then
		exports.NGCdxmsg:createNewDxMessage(source, "You cannot plant bombs while arrested", 255, 0, 0)
		return
	end
	if (getElementData(source, "tazed") == true) then
		exports.NGCdxmsg:createNewDxMessage(source, "You cannot plant bombs while tazed", 255, 0, 0)
		return
	end	
	if (getElementDimension(source) ~= 0 or getElementInterior(source) ~= 0) then
		exports.NGCdxmsg:createNewDxMessage(source, "You cannot plant bombs outside of the main world", 255, 0, 0)
		return
	end
    ...
    --local data = getElementData(mar)
        ...
        if playerMines [source] == 1 then
            exports.NGCdxmsg:createNewDxMessage ( "You already placed mines!", source, 255, 255, 0 )
        return
         end
		...
		exports.NGCdxmsg:createNewDxMessage ( "Mine has been planted!", source, 100, 0, 250 )

        -- setElementData(landmine,"number",
        else
			exports.NGCdxmsg:createNewDxMessage ( "You do not have enough money, you need $3000", source, 255, 0, 0 ) return end
		end
    end
end
...
addEventHandler ( "onPlayerQuit", getRootElement(), abuse )
