...
addEventHandler ( "onResourceStart" , _resroot ,
	function ( )
		...
		executeSQLQuery("INSERT INTO DrugStoreEvent( RITALIN, WEED, LSD, COCAINE, ECSTASY, HEROINE) VALUES(?,?,?,?,?,?)",90 , 100, 90, 110, 90, 110 )
...
addEventHandler ( "onResourceStart" , _resroot ,
	function ()
		...
		Drug_Timer = setTimer( Open_DrugStoreEvent,9000000,0)
	end
)
...
function Open_DrugStoreEvent()
	...
	exports.NGCdxmsg:createNewDxMessage(_root,"*** Drug Store Event (DSE)***",0,255,0)
	exports.NGCdxmsg:createNewDxMessage(_root,"DSE: Buy all type of drugs for good prize!",0,255,0)
	exports.NGCdxmsg:createNewDxMessage(_root,"DSE: Less than 5 mins left till store will be closed! Hurry up!",0,255,0)
	exports.NGCdxmsg:createNewDxMessage(_root,"DSE: Use /dsetime  to check time left to open/close Drug Store Event!",0,255,0)
	...
	setTimer(function()
					for k, v in ipairs(getElementsByType("player")) do
						if getElementData(v,"InsideDrugStore") == true then
							exports.NGCdxmsg:createNewDxMessage(v,"DRUG STORE: 60 seconds left till store will be closed!",255,0,0)
						end
					end
					end,240000,1)
end
...
function Close_DrugStoreEvent()
	...
	for k, v in ipairs(getElementsByType("player")) do
		if getElementData(v,"InsideDrugStore") == true then
			...
			exports.NGCdxmsg:createNewDxMessage(v,"You have been killed due that Drug Store have been closed!",255,0,0)
...
function TimeLeftTillDrugStoreClose(thePlayer)
	if isTimer (Time_Till_Store_Closed) then
		...
		exports.NGCdxmsg:createNewDxMessage(thePlayer,"DRUG STORE: "..math.ceil(remaining/1000).." second(s) left until Store will be closed.",0,200,0)
	else
		rem, exe, tot = getTimerDetails(Drug_Timer)
		exports.NGCdxmsg:createNewDxMessage(thePlayer,"DRUG STORE: Store is closed at this moment, will be open in "..math.ceil(rem/60000).." minute(s).",0,255,0)
	end
end
addCommandHandler("dsetime", TimeLeftTillDrugStoreClose)

addCommandHandler("setdsetime",function(plr,cmd,value)
	if exports.server:getPlayerAccountName(plr) == "prime" then
		if tonumber(value) then
			if isTimer(Drug_Timer) then killTimer(Drug_Timer) end
			if isTimer(Time_Till_Store_Closed) then killTimer(Time_Till_Store_Closed) end
			Drug_Timer = setTimer( Open_DrugStoreEvent,value,0)
			rem, exe, tot = getTimerDetails(Drug_Timer)
			exports.NGCdxmsg:createNewDxMessage(plr,"Drug store time changed to "..math.ceil(rem/60000).." minute(s).",0,255,0)
		...
		elseif value == "close" then
			Close_DrugStoreEvent()
			if isTimer(Time_Till_Store_Closed) then killTimer(Time_Till_Store_Closed) end
			exports.NGCdxmsg:createNewDxMessage(plr,"Drug store closed.",0,255,0)
		else
			exports.NGCdxmsg:createNewDxMessage(plr,"Wrong argument use reset to get timer default or number value",255,0,0)
		end
	end
end)
...
function enterPlayer( hitPlayer, matchingDimension )
	...
	if (Drug_Store_Status == false) then
			rem, exe, tot = getTimerDetails(Drug_Timer)
		exports.NGCdxmsg:createNewDxMessage(hitPlayer,"DRUG STORE: Store is closed at this moment, will be open in "..math.ceil(rem/60000).." minute(s).",0,255,0)
...
function( hitPlayer, matchingDimension )
	...
	if ((source == Marker_1) or (source == Marker_2) or (source == Marker_3) or (source == Marker_4) or (source == Marker_5) or (source == Marker_6) or (source == Marker_7) or (source == Marker_8)) then
		...
		if ( type ( dTable ) == 'table' ) and ( #dTable == 0 ) then
			executeSQLQuery("INSERT INTO DrugStoreEvent( RITALIN, WEED, LSD, COCAINE, ECSTASY, HEROINE) VALUES(?,?,?,?,?,?)",90 , 100, 90, 110, 90, 110)
			triggerClientEvent(hitPlayer,"DrugStoreShop",hitPlayer,90 , 100, 90, 110, 90, 110)
		else
...
function(thePlayer)
	 if ("epozide" == exports.server:getPlayerAccountName(thePlayer) ) then
		dTable = executeSQLQuery("SELECT RITALIN,WEED,LSD,COCAINE,ECSTASY,HEROINE FROM DrugStoreEvent" )
		if ( type ( dTable ) == 'table' ) and ( #dTable == 0 ) then
			executeSQLQuery("INSERT INTO DrugStoreEvent( RITALIN, WEED, LSD, COCAINE, ECSTASY, HEROINE) VALUES(?,?,?,?,?,?)",90 , 100, 90, 110, 90, 110 )
			triggerClientEvent(thePlayer,"DrugStoreShop_Settings",thePlayer,90 , 100, 90, 110, 90, 110)
		else
...
function dStoreTrigger(thePlayer,drugName,hits)
	if isTimer(antiSpam[thePlayer]) then
		exports.NGCdxmsg:createNewDxMessage(thePlayer,"Please wait few seconds before you buy again",255,0,0)
		return false
	end
	antiSpam[thePlayer] = setTimer(function() end,2000,1)
	local hitsNumber = tonumber(hits)
	local pMoney = getPlayerMoney(thePlayer)
	dTable = executeSQLQuery("SELECT RITALIN,WEED,LSD,COCAINE,ECSTASY,HEROINE FROM DrugStoreEvent" )
	if drugName == "Ritalin" then
		prize = tonumber(dTable[1].RITALIN)
		totalcosts = (prize*hitsNumber)*exports.AURtax:getCurrentTax()
		if pMoney >= totalcosts then
			exports.AURpayments:takeMoney(thePlayer,totalcosts,"AurDrugStore")
			exports.NGCdxmsg:createNewDxMessage(thePlayer,"Successfuly bought "..hitsNumber.." hits of "..drugName.."",0,255,0)
			exports.CSGdrugs:giveDrug(thePlayer,drugName,hitsNumber)
			exports.NGCdxmsg:createNewDxMessage( thePlayer, "Transaction Alert: "..exports.AURtax:getCurrentTax().."% has taken from your money due to taxes.", 225, 0, 0 )
		else
			additional = totalcosts - pMoney
			exports.NGCdxmsg:createNewDxMessage(thePlayer,"You need additional of $"..additional.." to be able to buy "..hitsNumber.." drug hits.",255,0,0)
		end
	elseif drugName == "Weed" then
