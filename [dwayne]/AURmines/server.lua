...
function mine (source)
    ...
    if isElement(source) then
        ...
        if playerMines[source] == 1 then
            exports.NGCdxmsg:createNewDxMessage ( "You already placed a Mine!", source, 255, 255, 0 )
        return
         end
       money = getPlayerMoney (source)
       if (money >= 2500) then
		exports.AURpayments:takeMoney(source, 2500,"AUR Mines")
		...
		table.insert(myTable,mar)
		table.insert(myTable,landmineCol)
		table.insert(myTable,landmine)
        ...
        exports.NGCdxmsg:createNewDxMessage ( "Mine has been planted!", source, 0, 0, 255 )
        else
		        exports.NGCdxmsg:createNewDxMessage ( "You do not have enough money!", source, 255, 0, 0 ) return end
