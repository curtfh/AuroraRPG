	...
	if row ~= false and row ~= nil and row ~= -1 then
		...
		if i ~= nil then
			if houseData[i][3] == true then
				...
				exports.NGCdxmsg:createNewDxMessage("This house is already visible on your map",255,255,0)
			else
				if isElement(blips[i]) then destroyElement(blips[i]) end
				blips[i] = createBlip(houseData[i][4],houseData[i][5],houseData[i][6],31)
				houseData[i][3] = true
				update()
				exports.NGCdxmsg:createNewDxMessage("Your house at '"..streetName.."' is now visible on your map",0,255,0)
			end
		end
	else
		exports.NGCdxmsg:createNewDxMessage("You didn't select a house!",255,255,0)
	end
end
	...
	if row ~= false and row ~= nil and row ~= -1 then
		...
		if i ~= nil then
			if houseData[i][3] == false then
				--do nothing its already showing
				exports.NGCdxmsg:createNewDxMessage("This house is already not visible on your map",255,255,0)
			else
				if isElement(blips[i]) then destroyElement(blips[i]) end
				houseData[i][3] = false
				update()
				exports.NGCdxmsg:createNewDxMessage("Your house at '"..streetName.."' is no longer visible on your map",0,255,0)
			end
		end
	else
		exports.NGCdxmsg:createNewDxMessage("You didn't select a house!",255,255,0)
