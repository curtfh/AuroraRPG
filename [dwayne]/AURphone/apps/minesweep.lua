...
function onRender()
	...
	for k,v in pairs(GAME_VAR.grid) do
		for k2,v2 in pairs(v) do
			if(v2 ~= "*") then
				...
				dxDrawRectangle(POS.X+(k2*GAME_VAR.boxW)-GAME_VAR.boxW+1,POS.Y+(k*GAME_VAR.boxH)-GAME_VAR.boxH+1,GAME_VAR.boxH-1,GAME_VAR.boxW-1,BlockColours[v2],true)
				dxDrawImage(POS.X+(k2*GAME_VAR.boxW)-GAME_VAR.boxW+1,POS.Y+(k*GAME_VAR.boxH)-GAME_VAR.boxH+1,GAME_VAR.boxH-1,GAME_VAR.boxW-1,	"Games/mine.png", 0, 0, 0, tocolor(255,255,255), true)
			end
			...
			if(GAME_VAR.flags[tostring(k..","..k2)]) then
				dxDrawImage(POS.X+(k2*GAME_VAR.boxW)-GAME_VAR.boxW+1,POS.Y+(k*GAME_VAR.boxH)-GAME_VAR.boxH+1,GAME_VAR.boxH-1,GAME_VAR.boxW-1,"Games/flag.png", 0, 0, 0, tocolor(255,255,255), true)
			end
	...
	drawBox(""..GAME_VAR.time,POS.X+10,POS.Y+(GAME_VAR.boxH*#GAME_VAR.grid)+10,50,"Games/clock.png")
	drawBox(""..GAME_VAR.mines,POS.X+SIZE.W-70,POS.Y+(GAME_VAR.boxH*#GAME_VAR.grid)+10,50,"Games/mine.png")
	...
	if(SelectedLevel ~= 1) then
		dxDrawImage(POS.X+(SIZE.W/2)-(dxGetTextWidth(str)/2)-(tw/2),POS.Y+(GAME_VAR.boxH*#GAME_VAR.grid)+52,14,24,"Phone/Games/al.png", 0, 0, 0, tocolor(255,255,255), true)
	end
