...
function openMoneyApp()
	...
	addEventHandler	("onClientGUIChanged", moneyGUI[4], setm, false)
...
function setm(element)
	...
	if ( txts ~= "" and tonumber( txts2 ) ) then
		guiSetText(moneyGUI[3],"Amount: $ "..exports.server:convertNumber(txts))
	end
end
...
function onSendPlayerMoney ()
	if ( spamProtect[localPlayer] ) and ( getTickCount()-spamProtect[localPlayer] < 30000 ) then
		exports.NGCdxmsg:createNewDxMessage( "Do not spam the send money function you are allowed to send money every 30 seconds!", 225, 0, 0 )
	else
		local theMoney = guiGetText( moneyGUI[4] )
		if ( string.match(theMoney,'^%d+$') ) then
			if tonumber(theMoney) >= 500 then
				if ( getPlayerMoney() >= tonumber( theMoney ) ) then
					...
					if ( playerName ) and ( getPlayerFromName( playerName ) ) then
						if ( string.len( tostring( theMoney ) ) ) < 9  then
					...
					exports.NGCdxmsg:createNewDxMessage( "You don't have enough money!", 225, 0, 0 )
				end
			else
				exports.NGCdxmsg:createNewDxMessage( "You are only allowed to send $500 or above", 225, 0, 0 )
			end
		end
	end
end
