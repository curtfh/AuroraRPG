...
[1] = {"Paramedic", "Paramedics"},
[2] = {"Pilot", "Civilian Workers"},
[3] = {"Mechanic", "Civilian Workers"},
[4] = {"Taxi Driver", "Civilian Workers"}
}
	...
	if ( row ) and ( column ) and ( phoneServices[row][1] ) then
		...
		if ( serviceOccupation ) and ( serviceTeam ) then
			if ( phoneSpam[serviceOccupation] ) and ( getTickCount()-phoneSpam[serviceOccupation] < 30000 ) then
				exports.NGCdxmsg:createNewDxMessage("Your call was not sent due to spam protection!", 225, 0, 0)
			else
				exports.NGCdxmsg:createNewDxMessage("You called to the " .. serviceOccupation .. " services!", 225, 0, 0)
