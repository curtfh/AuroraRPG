...
function replySMS(cmdName,...)
	if isElement(lastSender) then
		...
		exports.NGCdxmsg:createNewDxMessage("You have not recieved a SMS or the player left!",255, 0, 0)
	end
...
function drawUnreadMessages ()
	...
	if #unreadMessages >= 1 then
		...
		local x,y,width,height = (1120/1440)*sW, (205/900)*sH, (220/1920)*sW, 30
		dxDrawRectangle( x,y, width, height, tocolor( 0,0,0,200 ) )
		dxDrawText("Unread messages:( "..tostring(#unreadMessages).." )", x+40,y, x+width, y+height, tocolor( 255,0,0 ), 1, 'default','center','center' )
...
function ()
	...
	-- friend checkbox
	SMSGUI[2] = guiCreateCheckBox(BGX+(BGWidth)-90,BGY+(0.40962030887604*BGHeight),90,0.056031551212072*BGHeight,"Friends only",false,false)
	-- messages memo
	SMSGUI[3] = guiCreateMemo(BGX+(0.0064947726204991*BGWidth),BGY+(0.0026681690942496*BGHeight),0.99422937631607*BGWidth,0.40428396463394*BGHeight,"",false)
	guiMemoSetReadOnly ( SMSGUI[3], true )
	-- find players edit
	SMSGUI[4] = guiCreateEdit(BGX+(0.0030184460338205*BGWidth),BGY+(0.500695210695267*BGHeight),BGWidth,0.064036056399345*BGHeight,"",false)
	-- send msg edit
	...
	SMSGUI[7] = guiCreateCheckBox(BGX+(BGWidth)-180,BGY+(0.40962030887604*BGHeight),80,0.056031551212072*BGHeight,"Busy",false,false)
	guiSetFont(SMSGUI[2],"default-bold-small")
	guiSetFont(SMSGUI[7],"default-bold-small")
	...
	fillSMSPlayerGrid()
	guiEditSetMaxLength(SMSGUI[4], 10)
	guiEditSetMaxLength(SMSGUI[5], 100)
	for i=1,#SMSGUI do
	...
	addEventHandler("onClientGUIClick",SMSGUI[7],function()
		if source == SMSGUI[7] then
			if (guiCheckBoxGetSelected(SMSGUI[7])) then
				setElementData(localPlayer, "PlayerIsBusy", true)
				exports.NGCdxmsg:createNewDxMessage("Offline Mode: On", 0, 255, 0)
			else
				setElementData(localPlayer, "PlayerIsBusy", false)
				exports.NGCdxmsg:createNewDxMessage("Offline Mode: Off", 255, 0, 0)
			end
		end
	end)
end
...
function onSMSRecieve(theString, sender)
	if theString then
		...
		if sender and sender ~= localPlayer then
			...
			if SMSringtone then playSound("Ringtones/"..tostring(SMSringtoneNumber)..".mp3") end
		end
	end
end
