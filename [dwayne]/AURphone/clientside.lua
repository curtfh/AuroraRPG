...
exports.DENsettings:addPlayerSetting('phoneBackground', '2')
------ models thing exports.DENsettings:addPlayerSetting('phoneModel', '1')
...
------ models thing phonemodel = exports.DENsettings:getPlayerSetting('phoneModel')
...
homeWidth = 0.200390625*caseWidth
...
{ nil,nil,nil,nil,nil, "mark", false, false, false, false, "GPS" }, -- < WIP
{ nil,nil,nil,nil,nil, "Flappy", false, false, false, false, "Flappy birds" }, -- < WIP
{ nil,nil,nil,nil,nil, "browser", false, false, false, false, "Browser" }, -- < WIP
--{ nil,nil,nil,nil,nil, "Translator", false, false, false, false, "Translator" }, -- < WIP
--{ nil,nil,nil,nil,nil, "Reportss", false, false, false, false, "Reportss" }, -- < WIP
...
function createCase()
	------ models thing model = "phone" .. phonemodel .. ".png"
	local caseImage = guiCreateStaticImage(caseX, sH, caseWidth, caseHeight, "Icons/ngcphone.png", false )
	------ models thing local caseImage = guiCreateStaticImage(caseX, sH, caseWidth, caseHeight, "Models\\".. tostring(model), false )
	...
	local homeLabel = guiCreateLabel ( homeX, sH+(homeY-caseY), homeWidth, homeHeight, "Exit", false )
	guiSetProperty ( homeLabel, "AlwaysOnTop", "True" )
	guiSetFont(homeLabel,"sa-header")
	guiBringToFront(homeLabel)
	guiLabelSetColor(homeLabel,255,0,0)
	guiSetAlpha(homeLabel,0)
...
--and exports.server:isPlayerLoggedIn( localPlayer )
	if ( not phoneVisible ) and exports.server:isPlayerLoggedIn( localPlayer ) and not fading then
		--if isTimer(anti) then
		--	exports.DENdxmsg:createNewDxMessage("You are allowed to open iPhone one time each 10 seconds",255,0,0)
		--return false end
		--anti = setTimer(function() end,10000,1)
		--[[if(not areImagesReady()) then
			exports.NGCdxmsg:createNewDxMessage("Please wait while the files are downloading.",255,0,0)
			return
		end]]
		local elementStandingOn = getPedContactElement ( localPlayer )
		if elementStandingOn and not getElementType(elementStandingOn) == "vehicle" or elementStandingOn and not getElementType(elementStandingOn) == "object" then
			exports.NGCdxmsg:createNewDxMessage("You have to be on the ground to open the phoneNGCdxmsg",255,0,0)
			return
		elseif (not elementStandingOn and not isPedOnGround(localPlayer) and not isPedInVehicle(localPlayer)) then
			exports.NGCdxmsg:createNewDxMessage("You have to be on the ground to open the phone.",255,0,0)
			return
		end
		createCase()
		createApps()
		...
		--end
		slide('up')
		setElementData(localPlayer,"isPlayerPhone",true)
	...
	elseif isElement(phoneImages[1]) then
		...
		if not fading then
			...
			setElementData(localPlayer,"isPlayerPhone",false)
			closeApps()
...
function onMouseClick ()
	...
	--[[	if v[11] == "Notes" then
			if v[1] == source then
				exports.NGCdxmsg:createNewDxMessage("This application is disabled!",255,0,0)
				return false
			end
		end]]--
		if v[11] == "Flappy birds" then
			if v[1] == source then
				local x = getParts()
				if x < 128 then
	...
	for i=1,#apps do
		local v = apps[i]
		if v[11] == "Browser" then
			if v[1] == source then
				forceHide()
				exports.webbrowser:openBrowser()
			end
		end
	end
end
