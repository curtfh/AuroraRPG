...
-- GUI Code
local DENStreamWindow = guiCreateWindow( 618, 250, 319, 306, "AUR ~ MP3/Radio Streamer", false )
guiWindowSetSizable( DENStreamWindow, false )
local DENStreamLabel = guiCreateLabel( 10, 23, 293, 16, "Enter a MP3/Radio URL or select one:", false, DENStreamWindow )
...
local radioStations = {
	...
	[ "Slam!FM" ] = "http://www.true.nl/streams/slamfmlivestream.pls",
...
addEventHandler( "onClientGUIClick", DENStreamButton1,
	function ()
		if ( isStreaming ) then
			exports.NGCdxmsg:createNewDxMessage( "You are already streaming, stop the first stream first!", 225, 0, 0 )
		elseif ( getStreamURL () == "" ) or ( getStreamURL () == " " ) then
			exports.NGCdxmsg:createNewDxMessage( "Can't start stream, please enter a valid URL", 225, 0, 0 )
		elseif ((antiSpam) and (getTickCount() - antiSpam < 3000)) then
			exports.NGCdxmsg:createNewDxMessage( "Don't spam the buttons!", 225, 0, 0 )
		else
				isStreaming = true
...
addEventHandler( "onClientGUIClick", DENStreamButton2,
	function ()
		if ((antiSpam) and (getTickCount() - antiSpam < 3000)) then
			exports.NGCdxmsg:createNewDxMessage( "Don't spam the buttons!", 225, 0, 0 )
		elseif ( isStreaming ) then
			...
			exports.NGCdxmsg:createNewDxMessage( "You aren't even streaming!", 225, 0, 0 )
		end
	end, false
)
...
addEventHandler( "onClientPlayerStartStreamSound", root,
	function ( soundTable, aPlayer )
		...
		if (soundTable[8] == true) then  -- VIPs
			setSoundMaxDistance( theSound, 25 )
		else
		...
		streamsTable[ source ] = { theSound, soundTable[1], soundTable[2], soundTable[3], soundTable[4], soundTable[5], soundTable[6], soundTable[7], soundTable[8] }
		guiSetEnabled( DENStreamButton1, true ) guiSetEnabled( DENStreamButton2, true )
		if ( aPlayer == localPlayer ) then
			if not ( theSound ) then
				exports.NGCdxmsg:createNewDxMessage( "Stream URL seems not to work properly!", 225, 0, 0 )
			end
			isStreaming = true
		end
	end
	...
	function ( theTable )
		for thePlayer, soundTable in pairs ( theTable ) do
			if ( soundTable ) then
				...
				streamsTable[ thePlayer ] = { theSound, soundTable[1], soundTable[2], soundTable[3], soundTable[4], soundTable[5], soundTable[6], soundTable[7], soundTable[8]  }
			end
		end
	end
)
