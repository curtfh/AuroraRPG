...
function chase_aim(ped)
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "chasing" ) and (isElement(player)) and (getElementData (ped, "SAPDbot") == true) then
...
function chase_move (ped, oldTx, oldTy, oldTz, oldPx, oldPy, oldPz)
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "chasing" ) and (isElement(thetarget)) and (getElementData (ped, "SAPDbot") == true) then
			local pedhp = getElementHealth ( ped )
...
function chase_shoot(ped)
	if (isElement(ped)) then
		local player = getElementData ( ped, "target" )
		if (isElement(player)) and (getElementData (ped, "SAPDbot") == true) then
...
function enemy_check (ped)
	if (isElement(ped)) then
		if ( getElementData ( ped, "status" ) == "hunting" ) and (getElementHealth(ped)>0) and (getElementData (ped, "SAPDbot") == true) then
...
function enemy_check (ped)
	if (isElement(ped)) then
		if (getElementHealth(ped)>0) and (getElementData (ped, "SAPDbot") == true) then
			if (getElementData ( ped, "controller" ) == getLocalPlayer()) then
...
function aidamage ( attacker, weapon, bodypart )
	if (isElement(attacker)) and (getElementData (source, "SAPDbot") == true) and (getElementData ( source, "controller" ) == getLocalPlayer()) then
...
function follow_move (ped, oldTx, oldTy, oldTz, oldPx, oldPy, oldPz)
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "following" ) and (isElement(thetarget)) and (getElementData (ped, "SAPDbot") == true) then
			local pedhp = getElementHealth ( ped )
			local tarhp = getElementHealth ( thetarget )
			if pedhp > 0 and tarhp > 0 then
...
function follow_aim(ped)
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "following" ) and (getElementData (ped, "SAPDbot") == true) then
...
function follow_enemy_check (ped)
	if (isElement(ped)) then
		if ( getElementData ( ped, "status" ) == "following" ) and (getElementHealth(ped)>0) and (getElementData (ped, "SAPDbot") == true) then
			if (getElementData ( ped, "controller" ) == getLocalPlayer()) then
				local Px,Py,Pz = getElementPosition( ped )
				if isPedDucked ( ped ) then
					local Pz = Pz-1
...
function followshoot(ped)
	...
	elseif (getPedWeapon(ped) == 28) then
		if (distance < 40 ) then
			local length = math.random( 2100, 5500 )
			triggerServerEvent ( "pedShootTrigger", ped, "gun", length )
			if ped and isElement(ped) then setTimer ( setElementData, length+100, 1, ped, "follow_shooting", false ) end
		end
	elseif (getPedWeapon(ped) == 29) then
		if (distance < 55 ) then
			local length = math.random( 2000, 5500 )
	...
	for theKey,thePed in ipairs(aipeds) do
		if (isElement(thePed)) then
			if ( getElementData ( thePed, "status" ) == "hunting" ) or ( getElementData ( thePed, "status" ) == "waiting" ) then
				if (getElementType ( theElement ) == "player") and (getElementHealth(thePed)>0) and (getElementData (thePed, "SAPDbot") == true) then
...
function Streamin ()
	if getElementData (source, "SAPDbot") == true then
...
function wait_enemy_check (ped)
	if (isElement(ped)) then
		if ( getElementData ( ped, "status" ) == "waiting" ) and (getElementHealth(ped)>0) and (getElementData (ped, "SAPDbot") == true) then
			if (getElementData ( ped, "controller" ) == getLocalPlayer()) then
				local Px,Py,Pz = getElementPosition( ped )
				if isPedDucked ( ped ) then
					local Pz = Pz-1
...
function guard_enemy_check (ped)
	if (isElement(ped)) then
		if ( getElementData ( ped, "status" ) == "guarding" ) and (getElementHealth(ped)>0) and (getElementData (ped, "SAPDbot") == true) then
			if (getElementData ( ped, "controller" ) == getLocalPlayer()) then
				local Px,Py,Pz = getElementPosition( ped )
				if isPedDucked ( ped ) then
					local Pz = Pz-1
