...
function peddeath(ammo, attacker, weapon, bodypart)
	if (getElementData (source, "SAPDbot") == true) then -- if its a SAPDbot
		triggerEvent ( "aurj.onBotWasted", source, attacker, weapon, bodypart )
		if (attacker) then
		...
		if isTimer(bots[source]) then killTimer(bots[source]) end
		bots[source] = setTimer(function(s)
	...
	else -- if its a non SAPDbot ped
		if (attacker) then
			if (getElementType(attacker) == "ped") and (getElementData (attacker, "SAPDbot") == true) then
...
function playerdeath( ammo, attacker, weapon, bodypart )
	if (attacker) then
		if (getElementData (attacker, "SAPDbot") == true) then
--nothing
		end
	end
	local peds = getElementsByType ( "ped" )
	for theKey,thePed in ipairs(peds) do
		if (getElementData (thePed, "SAPDbot") == true) then
			if (getElementData ( thePed, "leader" ) ==  source ) then
				if (getElementData ( thePed, "status" ) == "following" ) then
...
function assigntarget ( player )
	if ( not wasEventCancelled() ) then
		if (isElement(player)) and (getElementData (source, "SAPDbot") == true) then
...
function changestatus (dataName)
	if getElementType ( source ) == "ped" and dataName == "status" and (getElementData (source, "SAPDbot") == true) then
...
function chase_move (ped, oldTx, oldTy, oldTz, oldPx, oldPy, oldPz)
	if (isElement(ped)) then
		if (getElementData ( ped, "status" ) ==  "chasing" ) and (getElementData (ped, "SAPDbot") == true) then
...
function pathHit ( hitElement, matchingDimension )
	if getElementType (hitElement) == "ped" and (getElementData (hitElement, "SAPDbot") == true) then
...
function makePedShoot(type, length)
	if (isElement(source)) then
		if getElementType (source) == "ped" and (getElementData (source, "SAPDbot") == true) and (getElementData (source, "AllowFire") ~= false ) then
			if type == "melee" then
...
function hunt_move (ped,oldX,oldY,oldZ) --this function keeps the ped running towards whatever element is set as its "targetpath" elementdata
	if (isElement(ped)) then
		if (getElementData ( ped, "status" ) == "hunting" ) and (getElementData (ped, "SAPDbot") == true) then
			local pedhp = getElementHealth ( ped )
			if pedhp > 0 then
...
function follow_move (ped, oldTx, oldTy, oldTz, oldPx, oldPy, oldPz)
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "following" ) and (isElement(leader)) and (getElementData (ped, "SAPDbot") == true) then
			local pedhp = getElementHealth ( ped )
...
function wait_mode(ped)
	if (isElement(ped)) then
		local pedhp = getElementHealth ( ped )
		if (getElementData ( ped, "status" ) ==  "waiting" ) and (getElementData (ped, "SAPDbot") == true) and (pedhp > 0) then
			triggerClientEvent ( "bot_Stop", ped )
		end
	end
end
...
function guard_move (ped, oldpx, oldpy, oldpz)
	if (isElement(ped)) then
		if (getElementData ( ped, "status" ) ==  "guarding" ) and (getElementData (ped, "SAPDbot") == true) then
...
function setBotHunt (theped)
	if (isElement(theped)) then
		if (getElementData (theped, "SAPDbot") == true) and (getElementData (theped, "status") ~= "hunting") then
			assigncontroller(theped)
			setElementData ( theped, "target", nil )
			setElementData ( theped, "leader", nil )
			setElementData ( theped, "status", "hunting" )
...
function setBotWait (theped)
	if (isElement(theped)) then
		if (getElementData (theped, "SAPDbot") == true) and (getElementData (theped, "status") ~= "waiting") then
			assigncontroller(theped)
			setElementData ( theped, "target", nil )
			setElementData ( theped, "leader", nil )
...
--checks the function commands to see if all neccesary parts are filled, sets defaults if not or returns false.
	...
	local SAPDbot = createPed (tonumber(skin),tonumber(x),tonumber(y),tonumber(z))--spawns the ped
	if (SAPDbot ~= false) then
		setElementHealth(SAPDbot,650)
		triggerEvent ( "onBotSpawned", SAPDbot )
		setTimer ( setElementData, 200, 1, SAPDbot, "SAPDbot", true ) -- makes it a bot
		setTimer ( setElementData, 200, 1, SAPDbot, "AllowFire", true ) -- makes it able to shoot when it wants
		setTimer ( assigncontroller, 300, 1, SAPDbot ) --sets the bots controller
		setTimer ( giveWeapon, 800, 1, SAPDbot, tonumber(weapon), 99999, true ) --gives the weapon
		setElementData(SAPDbot, "BotWeapon", tonumber(weapon))
		if team ~= nil then
			setElementData(SAPDbot, "BotTeam", team)
		end
		setTimer ( setElementInterior, 100, 1, SAPDbot, tonumber(interior)) --sets interior
		setTimer ( setElementDimension, 100, 1, SAPDbot, tonumber(dimension)) --sets dimension

		--sets the mode
		if mode == "waiting" then
			setTimer ( setElementData, 600, 1, SAPDbot, "status", "waiting")
		elseif mode == "following" then
			setTimer ( setElementData, 400, 1, SAPDbot, "leader", modesubject )
			setTimer ( setElementData, 600, 1, SAPDbot, "status", "following")
		elseif mode == "chasing" then
			setTimer ( setElementData, 400, 1, SAPDbot, "target", modesubject )
			setTimer ( setElementData, 600, 1, SAPDbot, "status", "chasing")
		elseif mode == "guarding" then
			setTimer ( setBotGuard, 400, 1, SAPDbot, x, y, z)
		else
			setTimer ( setElementData, 600, 1, SAPDbot, "status", "hunting")
		end
		return SAPDbot
	end
end
...
function setBotTeam(ped, team)
	if (isElement(ped)) then
		if (getElementData (ped, "SAPDbot") == true) and (isElement(team)) then
...
function getBotTeam(ped)
	if (isElement(ped)) then
		if (getElementData (ped, "SAPDbot") == true) then
...
function setBotWeapon(ped, weapon)
	if (isElement(ped)) and getWeaponNameFromID(tonumber(weapon)) ~= false then
		if (getElementData (ped, "SAPDbot") == true) then
			setTimer ( giveWeapon, 100, 1, SAPDbot, tonumber(weapon), 99999, true )
