...
function ( creator )
	if ( getElementData ( localPlayer, "isPlayerSWAT" ) ) then
		if ( getProjectileType( source ) == 16 ) or ( getProjectileType( source ) == 17 ) or ( getProjectileType( source ) == 18 ) or ( getProjectileType( source ) == 39 ) then
			...
			if ( creator == localPlayer ) then
				exports.DENdxmsg:createNewDxMessage( "Its not allowed to use projectiles inside the event!", 225, 0, 0 )
			end
	...
	function ( theKiller, weapon, bodypart )
		if source and getElementData(source,"bot") or getElementData(source,"botX") then
			if ( isElement( theKiller ) ) and getElementType(theKiller) == "player" and ( getPlayerTeam( theKiller ) ) and ( getElementType( theKiller ) == "player" ) then
				if ( getElementData( theKiller, "isPlayerSWAT" ) ) then
					if ( getTeamName( getPlayerTeam( theKiller ) ) == "SWAT" ) or ( getTeamName( getPlayerTeam( theKiller ) ) == "Military Forces" ) then
...
function onDrawHOEStats ()
	...
	for k,v in ipairs(getElementsByType("ped", resourceRoot)) do
		if getElementData(v,"bot") or getElementData(v,"botX") then
			if k >= 1 then
				TerroristCount = k
			else
				TerroristCount = 0
	...
	-- Rectangle
	dxDrawDxRectangle(sx*(1121.0/1440),sy*(300.0/900),sx*(300/1440),sy*(220.0/900),0,0,0,100,false)
	-- Top killers Law
...
function drawHandler()
	if getPlayerTeam(localPlayer) and getTeamName(getPlayerTeam(localPlayer)) == "SWAT" or getTeamName(getPlayerTeam(localPlayer)) == "Military Forces" then
		if getElementData(localPlayer,"isPlayerSWAT") == true then
...
blip = exports.customblips:createCustomBlip( 1779.78, -1258.52,30,30,"T.png",1000)
exports.customblips:setCustomBlipRadarScale(blip,1.3,1.3)
