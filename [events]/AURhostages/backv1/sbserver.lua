...
function peddeath(ammo, attacker, weapon, bodypart)
	if (getElementData (source, "swatbot") == true) then -- if its a swatbot
	...
	else -- if its a non swatbot ped
		if (attacker) then
			if (getElementType(attacker) == "ped") and (getElementData (attacker, "swatbot") == true) then
...
function playerdeath( ammo, attacker, weapon, bodypart )
	if (attacker) then
		if (getElementData (attacker, "swatbot") == true) then
--nothing
		end
	end
	local peds = getElementsByType ( "ped" )
	for theKey,thePed in ipairs(peds) do
		if (getElementData (thePed, "swatbot") == true) then
			if (getElementData ( thePed, "leader" ) ==  source ) then
				if (getElementData ( thePed, "status" ) == "following" ) then
...
function assigntarget ( player )
	if ( not wasEventCancelled() ) then
		if (isElement(player)) and (getElementData (source, "swatbot") == true) then
...
function changestatus (dataName)
	if getElementType ( source ) == "ped" and dataName == "status" and (getElementData (source, "swatbot") == true) then
...
function chase_move (ped, oldTx, oldTy, oldTz, oldPx, oldPy, oldPz)
	if (isElement(ped)) then
		if (getElementData ( ped, "status" ) ==  "chasing" ) and (getElementData (ped, "swatbot") == true) then
...
function pathHit ( hitElement, matchingDimension )
	if getElementType (hitElement) == "ped" and (getElementData (hitElement, "swatbot") == true) then
...
function makePedShoot(type, length)
	if (isElement(source)) then
		if getElementType (source) == "ped" and (getElementData (source, "swatbot") == true) and (getElementData (source, "AllowFire") ~= false ) then
			if type == "melee" then
...
function hunt_move (ped,oldX,oldY,oldZ) --this function keeps the ped running towards whatever element is set as its "targetpath" elementdata
	if (isElement(ped)) then
		if (getElementData ( ped, "status" ) == "hunting" ) and (getElementData (ped, "swatbot") == true) then
			local pedhp = getElementHealth ( ped )
			if pedhp > 0 then
...
function follow_move (ped, oldTx, oldTy, oldTz, oldPx, oldPy, oldPz)
	if (isElement(ped)) then
		...
		if (getElementData ( ped, "status" ) ==  "following" ) and (isElement(leader)) and (getElementData (ped, "swatbot") == true) then
			local pedhp = getElementHealth ( ped )
...
function wait_mode(ped)
	if (isElement(ped)) then
		local pedhp = getElementHealth ( ped )
		if (getElementData ( ped, "status" ) ==  "waiting" ) and (getElementData (ped, "swatbot") == true) and (pedhp > 0) then
			triggerClientEvent ( "bot_Stop", ped )
		end
	end
end
...
function guard_move (ped, oldpx, oldpy, oldpz)
	if (isElement(ped)) then
		if (getElementData ( ped, "status" ) ==  "guarding" ) and (getElementData (ped, "swatbot") == true) then
...
function setBotHunt (theped)
	if (isElement(theped)) then
		if (getElementData (theped, "swatbot") == true) and (getElementData (theped, "status") ~= "hunting") then
			assigncontroller(theped)
			setElementData ( theped, "target", nil )
			setElementData ( theped, "leader", nil )
			setElementData ( theped, "status", "hunting" )
...
function setBotWait (theped)
	if (isElement(theped)) then
		if (getElementData (theped, "swatbot") == true) and (getElementData (theped, "status") ~= "waiting") then
			assigncontroller(theped)
			setElementData ( theped, "target", nil )
			setElementData ( theped, "leader", nil )
...
--checks the function commands to see if all neccesary parts are filled, sets defaults if not or returns false.
	...
	local swatbot = createPed (tonumber(skin),tonumber(x),tonumber(y),tonumber(z))--spawns the ped
	if (swatbot ~= false) then
		setElementHealth(swatbot,650)
		triggerEvent ( "onBotSpawned", swatbot )
		setTimer ( setElementData, 200, 1, swatbot, "swatbot", true ) -- makes it a bot
		setTimer ( setElementData, 200, 1, swatbot, "AllowFire", true ) -- makes it able to shoot when it wants
		setTimer ( assigncontroller, 300, 1, swatbot ) --sets the bots controller
		setTimer ( giveWeapon, 800, 1, swatbot, tonumber(weapon), 99999, true ) --gives the weapon
		setElementData(swatbot, "BotWeapon", tonumber(weapon))
		if team ~= nil then
			setElementData(swatbot, "BotTeam", team)
		end
		setTimer ( setElementInterior, 100, 1, swatbot, tonumber(interior)) --sets interior
		setTimer ( setElementDimension, 100, 1, swatbot, tonumber(dimension)) --sets dimension

		--sets the mode
		if mode == "waiting" then
			setTimer ( setElementData, 600, 1, swatbot, "status", "waiting")
		elseif mode == "following" then
			setTimer ( setElementData, 400, 1, swatbot, "leader", modesubject )
			setTimer ( setElementData, 600, 1, swatbot, "status", "following")
		elseif mode == "chasing" then
			setTimer ( setElementData, 400, 1, swatbot, "target", modesubject )
			setTimer ( setElementData, 600, 1, swatbot, "status", "chasing")
		elseif mode == "guarding" then
			setTimer ( setBotGuard, 400, 1, swatbot, x, y, z)
		else
			setTimer ( setElementData, 600, 1, swatbot, "status", "hunting")
		end
		return swatbot
	end
end
...
function setBotTeam(ped, team)
	if (isElement(ped)) then
		if (getElementData (ped, "swatbot") == true) and (isElement(team)) then
...
function getBotTeam(ped)
	if (isElement(ped)) then
		if (getElementData (ped, "swatbot") == true) then
...
function setBotWeapon(ped, weapon)
	if (isElement(ped)) and getWeaponNameFromID(tonumber(weapon)) ~= false then
		if (getElementData (ped, "swatbot") == true) then
			setTimer ( giveWeapon, 100, 1, swatbot, tonumber(weapon), 99999, true )
