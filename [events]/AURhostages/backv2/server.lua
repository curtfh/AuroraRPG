...
function setMainEvent()
	...
	startingTime = setTimer(function() hostages = false delay() end,120000*60,1)
end
...
function createCaptures()
	...
	for k,v in ipairs(getElementsByType('player')) do
		if getElementData(v,"isPlayerSWAT") then
			exports.DENdxmsg:createNewDxMessage(v,"You have 5 mintues before terrorists bomb the area, find the C4 charges and deactivate them",0,255,0)
		end
	end
...
function createCaptures2()
	...
	for k,v in ipairs(getElementsByType('player')) do
		if getElementData(v,"isPlayerSWAT") then
			exports.DENdxmsg:createNewDxMessage(v,"You have 5 mintues to capture the points go find them and watch out from getting killed.",0,255,0)
		end
	end
end
...
function handleLastMission()
	for k,v in ipairs(getElementsByType("player")) do
		if getElementData(v,"isPlayerSWAT") then
			...
			setElementData(v,"isPlayerSWAT",false)
...
function setBattleConfig2()
	...
	for k,v in ipairs(pos2) do
		...
		if not mkr[ped] then
			mkr[ped] = createMarker(x,y,z-1,"cylinder",1.5,255,0,0,155)
		end
...
addEventHandler("onPedWasted",root,function()
	if source and getElementData(source,"kills") == true then
		for k,v in ipairs(peds) do
			if source == v then
				table.remove(peds,k)
...
addEventHandler("onPlayerQuit",root,function()
	if getElementData(source,"isPlayerSWAT") then
		...
		setElementData(source,"isPlayerSWAT",false)
		triggerClientEvent( "onChangeHOECount", root, #lawPlayers)
	end
end)
...
showTime = function( thePlayer )
	if getElementData(thePlayer,"Group") == "SWAT Team" or getElementData(thePlayer,"Group") == "Military Forces" or getElementData(thePlayer,"isPlayerPrime") then
		...
		elseif ( isTimer(rescueTimer) ) then
			...
			exports.DENdxmsg:createNewDxMessage( thePlayer, reason.." in " ..onCalcTimer( math.floor( timeLeft ) ), 235, 255, 0 )
		...
		elseif not isTimer(startingTime) and hostages == false then
			if ( isTimer(delayer) ) then
				...
				exports.DENdxmsg:createNewDxMessage( thePlayer, "Time left until "..robType.." can be started "..onCalcTimer ( math.floor( timeLeft ) ), 0, 255, 0 )
			end
