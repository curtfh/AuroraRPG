...
addEvent ("aurraces.startPreRace", true)
function startRacePre (raceName, checkpoints, racers)
	if (not racing) then
		...
		setElementData (localPlayer ,"aurraces.checkpointIndexLastHit", 0)
...
addEventHandler ("aurraces.startPreRace", root, startRacePre)

addEvent ("aurraces.startRace", true)
...
addEventHandler ("aurraces.startRace", root, startRace)

addEvent ("aurraces.stopRace", true)
function stopRace ()
	if (racing) then
		racing = false
		...
		playerFinished = false
		startRaceticks = false
		setElementData (localPlayer ,"aurraces.checkpointIndexLastHit", false)
		...
		fadeCamera (true)
		if (isTimer (changeCountDownColorTimer)) then killTimer (changeCountDownColorTimer) end
		setElementData (localPlayer, "aurraces.isFinished", false)
		drawPositions = false	
	end
end
addEventHandler ("aurraces.stopRace", root, stopRace)
...
if (not source) then source = marker end
	if (hitElement == localPlayer) then
		...
		if (not race_checkpoints_hit[source]) then
			local oldIndex = getElementData (localPlayer ,"aurraces.checkpointIndexLastHit")
			local newIndex = oldIndex + 1
			local nextMarkerIndex = newIndex + 1		
			setElementData (localPlayer ,"aurraces.checkpointIndexLastHit", newIndex)
...
function race_onHitFinish()
	triggerServerEvent ("aurraces.onPlayerFinish", localPlayer)
	playerFinished = true
end
...
function () 
	...
	stopRace()
	setElementData (localPlayer, "aurraces.isSignedUp", false) 
	if (getPedOccupiedVehicle (localPlayer)) then
		setVehicleDamageProof (getPedOccupiedVehicle (localPlayer), false)
		setElementFrozen (getPedOccupiedVehicle (localPlayer), false)	
	end
