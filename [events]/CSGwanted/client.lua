...
function fireWep(weapon, ammo, ammoInClip, fX, fY, fZ, hitElement)
    ...
    if exports.server:getPlayChatZone(hitElement) == "LV" then return end
...
function wasted(killer,wep)
    ...
    if isElement(killer) then
        if exports.server:getPlayChatZone(killer) == "LV" then return end
        if getElementType(killer) == "player" then
			...
			if getElementData(killer,"insideDuel") == true then return false end
            if exports.DENlaw:isLaw(killer) then return end
            ...
            if getElementType(killer) == "vehicle" then
                ...
                if getElementType(con) == "player" then
                    if not exports.DENlaw:isLaw(con) then
                        wanted(con,18)
                    end
                end
            end
...
if getElementData(source,"Rank") == "Butcher " then
     local h = getElementHealth(targetPlayer)
		if h > 0 then
			setElementHealth(targetPlayer,h-20)
		end
	else
	outputChatBox("TEST2")
end
...
function damageDetect(atker,wep,bodypart)
	...
	if atker and getElementType(atker) == "ped" then return end
    if exports.server:getPlayChatZone(atker) == "LV" then return end
    local veh = false
    ...
    if exports.DENlaw:isLaw(atker) then
        if exports.DENlaw:isLaw(localPlayer) then return end
    ...
    if veh == false then
        if wepsToI[wep] ~= nil then
			...
			local Area = exports.server:getPlayChatZone(atker)
			if Area ~= "LV" and (getTeamName(getPlayerTeam(atker)) == "Criminals" or getTeamName(getPlayerTeam(atker)) == "HolyCrap") and (getTeamName(getPlayerTeam(source)) == "Criminals" or getTeamName(getPlayerTeam(source)) == "HolyCrap") then
			return else
...
addEventHandler("onClientLocalPlayerDamage", localPlayer, damageDetect)
--181 191 284
...
function lowerWanted ()
    ...
    if wantedPts and (tonumber(wantedPts) > 0) then
        ...
        if copAround == false then
            setElementData(localPlayer, "wantedPoints", getElementData(localPlayer, "wantedPoints")-0.5)
        end
    end
end
...
--------------------------------
--[[
window = guiCreateWindow(0.3,0.2839,0.35,0.2956, "NGC ~ Your last committed crimes", true)
...
window = guiCreateWindow(165, 184, 475, 188, "NGC ~ Crimes", false)
guiWindowSetSizable(window, false)
guiSetVisible(window,false)
centerWindow(window)
lable1 = guiCreateLabel(10, 24, 455, 21, "Your last committed crimes", false, window)
guiSetFont(lable1, "default-bold-small")
guiLabelSetHorizontalAlign(lable1, "center", false)
theGrid = guiCreateGridList(12, 48, 308, 126, false, window)
lable2 = guiCreateLabel(328, 50, 137, 19, "My wanted points", false, window)
guiSetFont(lable2, "default-bold-small")
guiLabelSetHorizontalAlign(lable2, "center", false)
lable3 = guiCreateLabel(335, 76, 126, 23, "0", false, window)
guiSetFont(lable3, "default-bold-small")
guiLabelSetHorizontalAlign(lable3, "center", false)
btnExit = guiCreateButton(329, 143, 136, 31, "Close", false, window)
guiSetProperty(btnExit, "NormalTextColour", "FFAAAAAA")
guiGridListAddColumn( theGrid, "", 0.85 )

function show()
    guiSetVisible(window,true)
    showCursor(true)
	guiSetText(lable3,getElementData(localPlayer,"wantedPoints"))
end
...
function recUpdatedData(theTable,period)
	...
	wnt = getElementData(localPlayer,"wantedPoints")
	--[[if wnt then
    guiGridListSetItemText( theGrid, catRow, 1, "My Wanted Points: "..math.floor(wnt).."", true, false )
	end]]
...
function toggleJailWindow()
    if guiGetVisible(jailWindow) == true then
        ...
        local x2,y2,z2 =  917.09442138672,-2275.5766601563,739.86828613281
        if dim == 2 then
...
addEventHandler("onClientResourceStart",resourceRoot,
    function()
        GUIEditor_Label = {}
        jailWindow = guiCreateWindow(0.0266,0.2839,0.2734,0.2956,"AUR ~ Jail",true)
