...
local data = {
    ...
    [39] = {"Attempting to escape the prison",60},
...
LVcol = createColRectangle(675.86,516.25,2300,2500)
    ...
    if theStamp ~= currTimeStamp then
        ...
        for k,v in ipairs(getElementsByType("player")) do
            ...
            if exports.server:isPlayerLoggedIn(v) == true then
                ...
                exports.DENmysql:exec("UPDATE crimes SET crime=? WHERE userid=? AND timestamp=?",toJSON(accountToday[accName]),userid,theStamp)
...
addEventHandler("CSGwanted.assasinate",root,function(atker,wep,bodypart)
    ...
    if getTeamName(getPlayerTeam(source)) ~= "Staff" then
		...
		if getElementDimension(source) == 0 then
			if getElementData(source,"isPlayerArrested") then return end
			if getElementData(source,"isPlayerJailed") then return end
			if getElementData(source,"isPlayerProtected") then return end
            if getPedArmor(source) > 0 then setPedArmor(source,getPedArmor(source)-30) return end
            if getElementHealth(source) > 150 then
                setElementHealth(source,getElementHealth(source)-(math.random(50,70)))
			elseif getElementHealth(source) >= 50 and getElementHealth(source) < 150 then
				setElementHealth(source,getElementHealth(source)-(math.random(30,50)))
            elseif getElementHealth(source) < 50 then
...
function s1(qh,p)
    ...
    if #t == 0 then
		if accountToday[exports.server:getPlayerAccountName( p )] then
			md = toJSON(accountToday[exports.server:getPlayerAccountName( p )])
			if md then
				exports.DENmysql:exec("INSERT INTO crimes SET userid=?, crime=?, timestamp=?", exports.server:getPlayerAccountID( p ),md, exports.CSGpriyenmisc:getTimeStampYYYYMMDD())
			end
		end
...
function s2(qh,p)
    ...
    if #t == 0 then
		...
		if accountCurrentJailPeriod[exports.server:getPlayerAccountName( p )] then
			nb = toJSON(accountCurrentJailPeriod[exports.server:getPlayerAccountName( p )])
			if nb then
				exports.DENmysql:exec("UPDATE crimes SET crime=? WHERE userid=? AND timestamp=?",nb,exports.server:getPlayerAccountID(p),"11111111")
			end
		end
    end
...
addEventHandler("onPlayerWasted",root,function(_,killer)
	...
	--exports.CSGscore:takePlayerScore(source,1)
...
function sd2(qh,p)
    ...
    if t2 == nil then dbFree(qh) t2 = {} end
...
function enterVeh(p,i,jackedP)
    ...
    if isElement(jackedP) then
        ...
        if getTeamName(getPlayerTeam(p)) ~= "Staff" then
            ...
            if getPlayerWantedLevel(jackedP) >= 1 and exports.DENlaw:isLaw(p)  then
...
function respond(ps,cmdName,i)
    ...
    if type(i) ~= "number" then
        ...
        exports.NGCdxmsg:createNewDxMessage(ps,"Responding Syntax: /respond crimeReportID",255,255,0)
    ...
    if locs[i] ~= nil then
        ...
        if not exports.DENlaw:isLaw(ps) then

            exports.NGCdxmsg:createNewDxMessage(ps,"You are not a LAW enforcement officer!",255,0,0)
        ...
        if locs[i][1] ~= nil then
            ...
            if resToID[ps] ~= nil then
                ...
                exports.NGCdxmsg:createNewDxMessage(ps,"Your already responding to a Crime Report, type /rescancel to cancel it!",255,255,0)
            ...
            exports.NGCdxmsg:createNewDxMessage(ps,descriptions[i],0,255,0)
    ...
    exports.NGCdxmsg:createNewDxMessage(ps,"This Crime Report ID does not exist!",255,255,0)
...
function cancelRespond(ps)
    ...
    if resToID[ps] ~= nil then
        ...
        exports.NGCdxmsg:createNewDxMessage(ps,"You are no longer responding to Crime Report #"..resToID[ps].."",255,255,0)
    ...
    exports.NGCdxmsg:createNewDxMessage(ps,"You are no longer responding to any Crime Report",255,255,0)

    resToID[ps] = nil

    exports.CSGgps:resetDestination(ps)

    --exports.NGCdxmsg:createNewDxMessage(ps,"You already aren't responding to a Crime Report",255,255,0)
...
local dming = {}
local so = {}
addEvent("dontCallMyDMER",true)
addEventHandler("dontCallMyDMER",root,function()
	if dming[source] == true then return false end
	dming[source] = true
	so[source] = setTimer(function(p)
		dming[p] = false
	end,100,1,source)
end)
...
function detected(p,i,victim,cx,cy,cz)
		...
		if getTeamName(getPlayerTeam(p)) == "Military Forces" then return end
		if getTeamName(getPlayerTeam(p)) == "Government" then return end
		if getElementDimension(p) == 5000 then return end --- paintball / topCriminal event
		if getElementInterior(p) == 0 and getElementDimension(p) ~= 0 then return end


		if getElementData(p,"isPlayerRobbing") then return end
		if getElementData(p,"isPlayerInMotel") then return end
		if getElementData(p,"isPlayerDFRobbing") then return end
		if getElementData(p,"CW") then return end
		if getElementData(p,"ANTIDM") then return end
		--if dming[p] == true then return end
		local wp = getElementData(p,"wantedPoints")
		local wv = getElementData(victim,"wantedPoints")
		if exports.server:getPlayChatZone(source) ~= "LV" then
			if data[i][1] and wv == 0 and wp == 0 and not exports.DENlaw:isLaw(p) then return end
			if data[i][1] and wv ~= 0 and not exports.DENlaw:isLaw(p) then return end
			if data[i][1] and wv ~= 0 and wp == 0 and not exports.DENlaw:isLaw(p) then return end


		end
        if isElementWithinColShape(p,Military) == true then
			if getElementData(p,"Group") == "Military Forces" then
				if victim and getElementData(victim,"Group") == "Military Forces" then
					return
				end
			end
		end
        if (exports.server:getPlayChatZone(p) == "LV" or isElementWithinColShape(p,seaCol)) and i ~= 38 then
                ...
                elseif zoneInfo[p] == "law" then
                    ...
                    if exports.DENlaw:isLaw(victim) and exports.CSGnewturfing2:isPlayerInRegTurf(victim) == true then
		...
		if getTeamName(getPlayerTeam(victim)) == "Civilian Workers" then return false end
...
function warnLaw(msg,r,g,b)
    ...
    for k,v in pairs(law) do
        ...
        -- exports.NGCdxmsg:createNewDxMessage(v,"---Notice---",0,255,0)
...
addEventHandler("onPlayerJailed",root,function(tim)
    ...
    if isElement(arrestor) then
        ...
        if wantedPoints > 40 then scoreToGive=2 end

        if wantedPoints > 50 then scoreToGive=2 end

        if wantedPoints > 70 then scoreToGive=3 end

        if wantedPoints > 100 then scoreToGive=5 end
...
function (hitElement)
    ...
    setElementData ( hitElement, "isPlayerInLvCol", false )
...
if getElementData(source,"Rank") == "Butcher " then
	...
	local h = getElementHealth(targetPlayer)

    setElementHealth(targetPlayer,h-40)
...
addEventHandler("onPlayerStealthKill", getRootElement(), abortAllStealthKills)
