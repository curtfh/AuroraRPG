...
function givePlayerScore(p,score)
	...
	local new = math.max(scores[p]+score) -- for when someone uses a negative amount
	scores[p]=new
...
function takePlayerScore(p,score)
	if p == nil or score == nil or scores[p] == nil then return end
	local new = math.max(0,scores[p]-score)
	scores[p]=new
	setElementData(p,"playerScore",math.floor(new),true)
	setElementData(p,"sbPS",math.floor(new),true)
	return true
...
for k,v in ipairs(getElementsByType("player")) do
	if exports.server:isPlayerLoggedIn(v) == true then
		...
		dbQuery(loginCB,{v},exports.DENmysql:getConnection(),"SELECT score FROM accounts WHERE id=?",exports.server:getPlayerAccountID(v))
	end
end
...
addEventHandler("onPlayerLogin",root,function()
		...
		dbQuery(loginCB,{source},exports.DENmysql:getConnection(),"SELECT score FROM accounts WHERE id=?",accountID)
end)
