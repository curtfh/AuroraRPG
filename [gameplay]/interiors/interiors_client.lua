...
function animateMarkers()
	for i,marker in ipairs(getElementsByType("marker", resourceRoot, true)) do
		if not getElementData(marker, "originalZ") then
			local xz,yz,zz=getElementPosition(marker)
			setElementData(marker,"originalZ",zz)
		end
...
addEventHandler ( "doWarpPlayerToInterior",localPlayer,
	function ( interior, resource, id )
		...
		if getElementData(source,"tazed",true) and not getElementData(source,"isPlayerJailed") then
		exports.NGCdxmsg:createNewDxMessage("You can't enter an interior when tazed",source,255,0,0)
		return false end
		if exports.server:isPlayerDamaged(source) then
			exports.NGCdxmsg:createNewDxMessage("You can't enter an interior because you are just health effected.",source,255,0,0)
		return false end
		if getElementData(source,"armor",true) then
			exports.NGCdxmsg:createNewDxMessage("You can't enter an interior with the armor",source,255,0,0)
		return false end
		if getElementData(source,"isPlayerFlagger",true) then
			exports.NGCdxmsg:createNewDxMessage("You can't enter an interior with the armor",source,255,0,0)
		return false end
		toggleAllControls ( false, true, false )
		--fadeCamera ( false, 1.0 )

		local prox,proy,proz = getElementPosition(source)
		setElementData(source,"interiorZone",{prox,proy,proz})
		setTimer ( setPlayerInsideInterior, 100, 1, source, int,dim,rot,x,y,z, interior )
		blockPlayer = true
...
function setPlayerInsideInterior ( player, int,dim,rot,x,y,z, interior )
	if getElementData(player,"tazed",true) and not getElementData(player,"isPlayerJailed") then
		exports.NGCdxmsg:createNewDxMessage("You can't enter an interior when tazed",player,255,0,0)
	return false end
	if exports.server:isPlayerDamaged(player) and not getElementData(player,"isPlayerJailed") then
		exports.NGCdxmsg:createNewDxMessage("You can't enter an interior because you are just health effected.",player,255,0,0)
	return false end
	if getElementData(player,"armor",true) and not getElementData(player,"isPlayerJailed") then
	...
	if getElementInterior(player) == 0 then
		local prox,proy,proz = getElementPosition(player)
		triggerServerEvent("setInteriorReturnData",player,prox,proy,proz)
	else
		setElementData(player,"interiorZone",false)
		triggerServerEvent("setInteriorStopReturnPlayer",player)
	end
	...
	setTimer ( function(p) if isElement(p) then setElementFrozen(localPlayer,false) setCameraTarget(p) end end, 200,1, player )
	...
	triggerServerEvent ( "onPlayerEnterInterior", player, dim,int )
end
