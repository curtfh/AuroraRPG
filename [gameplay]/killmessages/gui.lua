...
function drawLine ( message, x,y, align, line, r, g, b, font, scale )
	...
	for i,part in ipairs(message) do
		if type(part) == "string" then
			...
			if (i == 1 and type(message) == "table" and message[6]) then
				if (getPlayerKillStreak(part)) then
					local text2 = dxText:create ( getPlayerKillStreak(part), width, y, false )
					text2:font ( font )
					text2:scale ( scale )
					text2:type("shadow",1)
					text2:align"left"
					text2:color ( 255, 255, 255 )
					table.insert ( contentMessages[line], text2 )
					width = width + text2:extent()
				end
			end
...
function()
	for line,originalTick in pairs(fadingLines) do
		...
		if not tickDifference or not endTime or tickDifference > endTime then
			destroyLine ( line )
			setLineAlpha ( line, 1 )
...
function destroyLine ( line )
	if (not contentMessages[line]) then
		contentMessages[line] = {}
		return true
	end
