...
function KillMessages_onPlayerWasted ( totalammo, killer, killerweapon, bodypart )
	...
	elseif killer and isElement(killer) and getElementType(killer) == "ped" then
		if getElementData(killer,"isPedWife") then
			...
			if owner then
				...
				kr,kg,kb = getTeamColor ( getPlayerTeam ( owner ) )
				outputKillMessage ( source, wr,wg,wb,killer,kr,kg,kb,getPedWeapon(killer) )
			end
		end
	else
		local triggered = triggerEvent ( "onPlayerKillMessage", source,false,killerweapon,bodypart )
