...
scoreboardDimensions = { ["width"] = 1, ["height"] = 0, ["phase"] = 1, ["lastSeconds"] = 0 }
...
setTimer(function()
	...
	if showingRank2 == true then showingRank2=false else showingRank2=true end
	...
	if #scoreboardColumns ~= 0 then
		...
		if (currentTick - scoreboardTicks.lastUpdate > scoreboardTicks.updateInterval and (scoreboardToggled or scoreboardForced)) or forceScoreboardUpdate then
			...
			for key, player in ipairs( players ) do
				--if not (exports.server:getPlayerAccountName(player) == "darknes") then return false end
				...
				if (getElementData(player, "AURcurtmisc2.invisible") == false) then 
			...
			if (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) and not serverInfo.forcehideteams then
				...
				for key, team in ipairs( teams ) do
						...
						if column.name == "name" then
							...
							--[[if teamName == "Criminals" then
								...
								for k,v in pairs(getElementsByType("player")) do
									...
									if getElementData(v,"Occupation") == "Criminal" then
								...
								teamMemberCount = (j)
							--	outputDebugString(j.."||"..#noobs)
							elseif teamName == "Criminal Organization" then

								local i = 0
								local noobs = {}
								for k,v in pairs(getElementsByType("player")) do
									if getElementData(v,"Occupation") == "Criminal Organization" then
										i=i+1
									end
								end
							...
							if teamName == "Criminal Organization" then --HERE VALID or teamName == "The Terrorists" then

							--	teamName = ""..teamName .. ""

								teamName = ""..teamName .. " (" .. teamMemberCount .. "" .. iif( teamMemberCount == "1", "", "" ) .. ")"
					...
					--[[if getTeamName(team) == "Criminal Organization" then
						...
						for k,v in pairs(players) do
							...
							if gr == "Criminal Organization" then
								if getElementData(v,"Occupation") == "Criminal Organization"  then
									table.insert(t,v)
								end
							end
					...
					elseif getTeamName(team) == "Criminals" then
						...
						for k,v in pairs(players) do
							...
							local pSkin = getElementData(v,"Occupation")

							local gr = getElementData(v,"Group")

							if ( gr ~= "Criminal Organization" or (pSkin ~= "Criminal Organization")) then -- HERE VALID ( gr ~= "The Terrorists" or (pSkin ~= "The Terrorists")) then
								table.insert(X,v)
							end
						end
					...
					if (getElementData(player, "AURcurtmisc2.invisible") == false) then 
						...
						for key, column in ipairs( scoreboardColumns ) do
							...
							elseif column.name == "Ping" then
								...
								if (type(getElementData(player, "serverlocationdifference")) == "integer") then
									content = getPlayerPing( player )-getElementData(player, "serverlocationdifference")
								else 
									content = getPlayerPing( player )
								end 
							...
							elseif column.name == "Rank" then
								...
								if exports.DENlaw:isLaw(player) == true then
									...
									if showingRank2 == true then

										local skill2 = getElementData(player,"ExRank")
										if skill2 then
											content = getElementData( player, "ExRank" )
										end
		...
		if scoreboardDrawn then
			...
			if serverInfo.players and showServerInfo then
				local players = getElementsByType( "player" )
				local text = "Players Online: " .. tostring( #players )
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content and column.name == "name" then
							...
							if useColors then
								...
								---Group color of CO
								if exports.AURgroups:isGroup(element,gr,"crim") then r,g,b = exports.AURgroups:getGroupColor(gr) end
								if exports.AURgroups:isGroup(element,gr,"law") then r,g,b = exports.AURgroups:getGroupColor(gr) end
								if getPlayerTeam(element) and getTeamName(getPlayerTeam(element)) == "CS:GO" then
									if getElementData(element,"CS:GO Team") == "Counter-Terrorists" then
										r,g,b = 0,150,200
									elseif getElementData(element,"CS:GO Team") == "Terrorists" then
										r,g,b = 255,0,0
									end
								end
								---HERE VALID if gr == "The Terrorists" and getElementData(element,"Occupation") == "The Terrorists" and getPlayerTeam(element) == getTeamFromName("Criminals") then r,g,b = 255,215,0 end
							end
						...
						if content then
							...
							elseif column.name == "Loc" then
								...
								if content == "Unknown" then

										dxDrawText( "N/A", topX+theX+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )

										dxDrawText( "N/A", topX+theX, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )

									else

										dxDrawImage( topX+theX, y+s(1), 16, 11, content, 0, 0, 0, cWhite, drawOverGUI )

										dxDrawText( getElementData(element,"Country") or "N/A", topX+theX+18+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )

										dxDrawText( getElementData(element,"Country") or "N/A", topX+theX+18, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
