...
local firePlaces = {
	...
	---{-1988.7095947266,88.136123657227,27.6875,289.34582519531},
	{-1817.96, 142.17, 15.1},
	{-1817.96, 142.17, 15.1},
	{-2885.11, 460.5, 4.91},
    ...
    for k,v in pairs(getElementsByType("player")) do
        if isInFireFighterMode(v) then
			triggerClientEvent(v,"CSGfireStarted",v,x,y,z)
             exports.NGCdxmsg:createNewDxMessage(v,"A Fire has broke loose at "..zoneName.."! Requesting nearby Fire Fighters!",255,255,0)
        else
...
function isInFireFighterMode(p)
	...
	if ((getTeamName(team)) == "Civilian Workers" and exports.server:getPlayerOccupation(p) == "Firefighter") then
		return true
	else
		return false
	end
...
setTimer(startFire,5000,1,false)
        ...
        for k,v in pairs(activeFires) do
            if v == true then
                ...
                triggerClientEvent(player,"CSGfireStarted",player,x,y,z)
                 exports.NGCdxmsg:createNewDxMessage(player,"Hurry, a Fire is in progress at "..zoneName.."!",255,255,0)
            end
        end
end
...
function quitJob(player)
    ...
    -- exports.NGCdxmsg:createNewDxMessage(player,message,255,255,0)
end
...
function CSGfireHitFire(fire)
	...
	exports.AURpayments:addMoney(source,500,"Custom","Misc",0,"AURfire Firefighter")
	exports.CSGranks:addStat(source,1)
	exports.CSGscore:givePlayerScore(source,2)
    fires[fire] = fires[fire] - 100
end
...
function CSGfireHitFireByTruck(ped)
    for k,v in pairs(peds) do
        if k == ped then
            fires[v] = fires[v] - 40
		--	exports.CSGranks:addStat(source,0.1)
			---exports.AURpayments:addMoney(source,250,"Custom","Misc",0,"AURfire Firefighter")
            return
        end
    end
end
...
function enterFire(e)
	if e and isElement(e) and getElementDimension(e) == 0 then
		...
		if getElementType(e) == "player" then
			---setElementData(e,"isPlayerFireFighter",true)
...
function leaveFire(e)
	if e and isElement(e) then
		...
		if getElementType(e) == "player" then
			setElementData(e,"isPlayerFireFighter",false)
		elseif getElementType(e) == "vehicle" then
			--setElementHealth(e,math.random(20,100))
		end
    end
...
--[[
addEventHandler("onPedWasted",root,function(am,attacker,wep)
	if getElementData(source,"firePed") then
		outputDebugString("0")
		if getElementType(attacker) == "player" then
			outputDebugString("1")
			if not isPedInVehicle(attacker) then
				outputDebugString("2")
				if wep == 42 and getElementData(attacker,"Occupation") == "Firefighter" and getTeamName(getPlayerTeam(attacker)) == "Civilian Workers" then
					outputDebugString("3")
					for k,v in pairs(peds) do
						if k == source then
							outputDebugString("5")
							fires[v] = fires[v] - 100
							table.remove(peds,k)
							if isElement(k) then destroyElement(k) end
							exports.AURpayments:addMoney(attacker,350,"Custom","Misc",0,"AURfire Firefighter")
							exports.CSGranks:addStat(attacker,1)
						end
					end
				end
			end
		elseif getElementType(attacker) == "vehicle" then
			...
			if getElementData(player,"Occupation") == "Firefighter" and getPlayerTeam(player) and getTeamName(getPlayerTeam(player)) == "Civilian Workers" then
				if getElementModel(attacker) == 407 then
					for k,v in pairs(peds) do
						if k == source then
							fires[v] = fires[v] - 100
							table.remove(peds,k)
							if isElement(k) then destroyElement(k) end
							exports.AURpayments:addMoney(player,250,"Custom","Misc",0,"AURfire Firefighter")
							exports.CSGranks:addStat(player,1)
						end
					end
				end
			end
            ...
            if activeFires[k] == true then
                ...
                for k3,v3 in pairs(getElementsByType("player")) do
                    if isInFireFighterMode(v3) then
						exports.NGCdxmsg:createNewDxMessage(v3,"The fire at "..zoneName.." has been put out!",0,255,0)
