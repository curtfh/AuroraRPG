...
local limit = {}
local records = {}
records = fromJSON(tostring('[ { "Barracuda": [ "Dwayne", 11 ], "Tuna": [ "LCPL-Epicman[A-MF]", 11 ], "Squid": [ "Rig-16", 3792 ], "Red Snapper": [ "IL-0*GODFather|INV", 44 ], "Amberjack": [ "Rig-16", 110 ], "Mackeral": [ "~|T|~AlNhDe", 39 ], "Sea Bass": [ "Abdo|D", 11 ], "Lobster": [ "Dredd|TSF", 36 ], "Sail Fish": [ "Smile^|D", 45 ], "Grouper": [ "SGT|USS|-OsKar\/*", 40 ], "Trout": [ "TA-Wolf#B[FIA]", 11 ], "Turtle": [ "Ankur|D", 105 ], "Eel": [ "TheBenja^", 11 ], "Pike": [ "~|T|~AlNhDe", 43 ], "Shark": [ "Zod", 3854 ], "Catfish": [ "TA-Wolf#B[FIA]", 10 ], "Swordfish": [ "UA*L0-Luke[Sm]", 249 ], "Pirahna": [ "UA*[TSF]ViTo|>Re", 10 ], "Blue Marlin": [ "The.V[Sm]", 3018 ] } ]'))
...
--addCommandHandler("ngcfishbonustest",function()
--	newBonus()
--	announceBonus()
--end)
...
function sendData(p)
	...
	if ele then
	local fni = exports.DENstats:getPlayerAccountData(ele,"fishstat")
	if fni then
	local t = fromJSON(fni)
...
local lo = exports.DENstats:getPlayerAccountData(source,"fishstat")
if lo then
	local t0 = fromJSON(lo)
...
function CSGfishSetStat(stat,value,p,bool,st,rankPts)
	...
	local dk = exports.DENstats:getPlayerAccountData(source,"fishstat")
	if dk then
	local t = fromJSON(dk)
...
function CSGfishBuy(am,money)
	if getPlayerMoney(source) >= money then
		...
		local kj = exports.DENstats:getPlayerAccountData(source,"fishstat")
		if kj then
		local t = fromJSON(kj)
		--local t = fromJSON(exports.DENstats:getPlayerStats(source,"fishstat"))
...
--addCommandHandler("ngcaddfishrankpts",addRankPts)
