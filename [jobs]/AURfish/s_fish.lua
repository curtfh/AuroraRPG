...
local limit = {}
local records = {}
records = fromJSON(tostring('[ { "Barracuda": [ "Epozide", 11 ], "Tuna": [ "Sensei", 11 ], "Squid": [ "Rig-16", 3792 ], "Red Snapper": [ "Noki", 44 ], "Amberjack": [ "Rig-16", 110 ], "Mackeral": [ "~|T|~AlNhDe", 39 ], "Sea Bass": [ "Abdo|D", 11 ], "Lobster": [ "[CSG]Dredd|TSF", 36 ], "Sail Fish": [ "Smile^|D", 45 ], "Grouper": [ "SGT|USS|-OsKar\/*", 40 ], "Trout": [ "TA-Wolf#B[FIA]", 11 ], "Turtle": [ "Ankur|D", 105 ], "Eel": [ "[CSG]TheBenja^", 11 ], "Pike": [ "~|T|~AlNhDe", 43 ], "Shark": [ "Zod", 3854 ], "Catfish": [ "TA-Wolf#B[FIA]", 10 ], "Swordfish": [ "UA*L0-Luke[Sm]", 249 ], "Pirahna": [ "UA*[TSF]ViTo|>Re", 10 ], "Blue Marlin": [ "The.V[Sm]", 3018 ] } ]'))
...
function CSGfishGoodCatch(i)
	if i == 1 then -- money suitcase
		...
		exports.AURpayments:addMoney(source,m,"Custom","Fisherman",0,"AURfish")
		 exports.NGCdxmsg:createNewDxMessage(source,"Caught a money suitcase! Found $"..m.."",0,255,0)
	...
	elseif i == 2 then -- weapon suitcase
		 exports.NGCdxmsg:createNewDxMessage(source,"Found a weapon suitcase!",255,255,0)
		 exports.NGCdxmsg:createNewDxMessage(source,"But you dont want it so you threw it back!",255,255,0)
	elseif i == 3 then -- drug suitcase
		 exports.NGCdxmsg:createNewDxMessage(source,"Found a drug suitcase!",255,255,0)
		 exports.NGCdxmsg:createNewDxMessage(source,"But you dont want it so you threw it back!",255,255,0)
		-- exports.NGCdxmsg:createNewDxMessage(source,"drug suit case TODO",255,255,0)
	elseif i == 6 then -- treasure chest
		local m = math.random(1500,10000)
		--givePlayerMoney(source,m)
		exports.AURpayments:addMoney(source,m,"Custom","Fisherman",0,"AURfish")
		 exports.NGCdxmsg:createNewDxMessage(source,"Caught a treasure chest! Found $"..m.."",0,255,0)
		triggerClientEvent(source,"CSGfishAddStat",source,"$ Money earned from non-selling fish",m)
		triggerClientEvent(source,"CSGfishAddStat",source,"$ Money earned from fishing overall",m)
	end
end
...
function announceBonus()
	for k,v in pairs(getElementsByType("player")) do
		if exports.server:isPlayerLoggedIn(v) then
			...
			exports.NGCdxmsg:createNewDxMessageBot(v,"Be the first to catch it!",0,255,0)
			exports.NGCdxmsg:createNewDxMessageBot(v,"#33CCCCToday's Bonus Fish is a #00FF33"..bonusName.."#33CCCC, valued at #33FF00$"..bonusValue.."#33CCCC. (Minimum Weight "..bonusWeight.." Kg)",0,255,0)
...
function sendData(p)
	local ele = p or source
	local t = nil
	if (exports.DENstats:getPlayerAccountData(ele,"fishstat")) then
		t = fromJSON(exports.DENstats:getPlayerAccountData(ele,"fishstat"))
	end
	if not t then
...
function recUpdate(t,permits)
	local t0 = nil
	if (exports.DENstats:getPlayerAccountData(source,"fishstat")) then
		t0 = fromJSON(exports.DENstats:getPlayerAccountData(source,"fishstat"))
	end
...
addEventHandler("onPlayerLogin",root, function()
	local p = source
	setTimer(function(p) if (not isElement(p)) then return false end sendData(p) sendRecordInfo(p) end,5000,1, p)
end)
...
function newRecord(fish,weight)
	...
	if records[fish][2] < weight then
		...
		records[fish][1] = getPlayerName(source)
		 exports.NGCdxmsg:createNewDxMessage(source,"Congratulations! You have beat the "..fish.." record!",0,255,0)
		 exports.NGCdxmsg:createNewDxMessage(source,"Previous Record: "..prev.." Kg. New Record: "..weight.." Kg",0,255,0)
		sendRecordInfo()
	end
end
...
function CSGfishSetStat(stat,value,p,bool,st,rankPts)
	...
	local t = nil
	if (exports.DENstats:getPlayerAccountData(source,"fishstat")) then
		t = fromJSON(exports.DENstats:getPlayerAccountData(source,"fishstat"))
	end
...
function CSGfishBuy(am,money)
	if getPlayerMoney(source) >= money then
		...
		exports.AURpayments:takeMoney(source,money,"AURfish")
		local t = nil
		if (exports.DENstats:getPlayerAccountData(source,"fishstat")) then
			t = fromJSON(exports.DENstats:getPlayerAccountData(source,"fishstat"))
		end
		...
		--exports.DENstats:setPlayerStats(source,"fishstat",toJSON(t),true)
		sendData(source)
		 exports.NGCdxmsg:createNewDxMessage(source,"Bought a $"..money.." "..am.." fish permit.",0,255,0)
		 exports.NGCdxmsg:createNewDxMessage(source,"New Total Fish Permit: "..new.."",0,255,0)
	else
		 exports.NGCdxmsg:createNewDxMessage(source,"You can't afford $"..money.." "..am.." permit.",255,0,0)
	end
end
...
local accName = exports.server:getPlayerAccountName(source)
	...
	if limit[accName] < 0 then
		 exports.NGCdxmsg:createNewDxMessage(source,"You've sold us too much fish! You are damaging AUR's ocean life!",0,255,0)
		 exports.NGCdxmsg:createNewDxMessage(source,"You will get wanted if you sell us anymore for this hour.",0,255,0)
	...
	if limit[accName] < 0 then
		 exports.NGCdxmsg:createNewDxMessage(source,"You've sold us too much fish! You are damaging AUR's ocean life!",0,255,0)
		 exports.NGCdxmsg:createNewDxMessage(source,"You will get wanted if you fish anymore for this hour.",0,255,0)
	end
	...
	exports.AURpayments:addMoney(source,tot,"Custom","Fisherman",0,"AURfish")
	local scorePer3000=1
	local multi = tot/3000
	exports.CSGscore:givePlayerScore(source,scorePer3000*multi)
	 exports.NGCdxmsg:createNewDxMessage(source,"Sold a "..we.." Kg "..name.." for $"..val.." + Rank Bonus $"..rankBonus.."",0,255,0)
end
...
--[[function addRankPts(ps,cmdName,arg1,amt)
	...
	if isElement(p) == true then
		...
		outputChatBox("Gave "..arg1.." "..amt.." Fisherman Rank Points",ps,0,255,0)
		 exports.NGCdxmsg:createNewDxMessage(p,""..getPlayerName(ps).." has corrected your Fisherman Rank Points",0,255,0)
		 exports.NGCdxmsg:createNewDxMessage(p,""..amt.." Rank Points Added",0,255,0)
	else
