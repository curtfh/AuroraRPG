...
function onMTstart()
	outputDebugString("Started AUR driver mission")
	--
	setUpMarkers()
	--
...
function setUpMarkers()
	...
	if ( transPosz and type(transPosz) == "table" ) then
						...
						function onHit(hitter)
							...
							if not tFlood[hitter] == true then
								if not missionOn then
									...
									exports.NGCdxmsg:createNewDxMessage(hitter,"This mission is not available now.",255,255,255,true)
								end
							else
									exports.NGCdxmsg:createNewDxMessage(hitter,"You can't do the mission now, try again later!",255,255,255,true)
							end
							end
						end 
		...
		exports.NGCdxmsg:createNewDxMessage(root,"Mission is available now !",255,0,0,true)
		missionOn = false
end

function showTransWin(plr) -- showing the Driver window
	if plr then
		triggerClientEvent(plr,"showTransWin",plr)
	end
end

function startDriverMission(plr) -- starting new Driver mission
	if plr then
		missionOn = true
		
		exports.NGCdxmsg:createNewDxMessage("Driver Mission is available now, try it now and gain money!",255,255,255,true)
		--
		--easytext(plr,05551, "Driver Mission Started", 5 * 1000, 0.5, 0.7, 0, 255, 0, 255,2)
		--
		fadeCamera(plr,false)
		setTimer(function() fadeCamera(plr,true,2)
			x,y,z = getElementPosition(plr)
			firstX,firstY,firstZ = x,y,z
				transVeh = createVehicle(Driver_CAR_MODEL,x-10,y,z+1)
				transPed = createPed(Driver_PED_MODEL,x,y,z)
				if transVeh and transPed then
					warpPedIntoVehicle(transPed,transVeh,3)
					setTimer(function()
							easytextout(plr,05551, "Get in the car", 1000 * 1000, 0.5, 0.7, 0, 255, 0, 255,2)
												...
												function onVehEnter(enterer,seat)
													...
													randomDrop = math.random(#dropPosz)
														if randomDrop then
															...
															x,y,z = getElementPosition(plr)
															theDriverPrize = math.floor ( getDistanceBetweenPoints3D(x,y,z,dropX,dropY,dropZ) / 2 )
													...
													--
													easytextout(plr,05551, "Go to the D blip to earn money", 5 * 1000, 0.5, 0.8, 0, 255, 0, 255,2)
...
addEvent("startDriverMission",true)
addEventHandler("startDriverMission",root,startDriverMission)
...
if Player then
	...
	if Won == true then -- If he won the mission
		...
		easytextout(Player,05551, "Driver Mission Done ! , Earned  15000$", 8 * 1000, 0.5, 0.7, 0, 255, 0, 255,2)
		exports.NGCdxmsg:createNewDxMessage(root,"Successfully, "..getPlayerName(Player).."  Has Done the Driver misssion and earned 15000$",0,255,255,true)
		--
		triggerClientEvent(Player,"missionDone",Player,true)
	else
		triggerClientEvent(Player,"missionDone",Player,false)
		easytextout(Player,05551, "Driver Mission Failed ! ", 8 * 1000, 0.5, 0.7, 255, 0, 0, 255,2)
	end
