...
function preDraw()
    ...
    if h == 16 and justAD == false then
		justAD = true
		exports.NGCdxmsg:createNewDxMessage(root,"NGC Lotto 720 -- The next jackpot is an estimated $"..jackpot.."",0,255,0)
		exports.NGCdxmsg:createNewDxMessage(root,"Type /lotto 1 - 30 to play today!",0,255,0)
...
function noWinner(num)
    if jackpot < 200000 then
        ...
        exports.NGCdxmsg:createNewDxMessage(root,"NGC Lotto 720 -- The winning number was: "..num.."",0,255,0)
        exports.NGCdxmsg:createNewDxMessage(root,"No lotto winner -- The jackpot has been increased to $"..jackpot.."",0,255,0)
    data = {}
end
...
function winner(p,num)
    ...
    exports.NGCdxmsg:createNewDxMessage(root,"NGC Lotto 720 -- #FFFFFF The winning number was: "..num.."",0,255,0)
    exports.NGCdxmsg:createNewDxMessage(root,"Congratulations to "..winnerName.." for winning the $"..jackpot.." jackpot!",0,255,0)
...
function doCommandLotto(ps,commandName,arg1)
    ...
    if (arg1) then
        ...
        if type(num) == "number" then
            if getPlayerMoney(ps) < 500 then  exports.NGCdxmsg:createNewDxMessage(ps,"You need $500 to order a CSG Lotto 720 Ticket. You only have $"..getPlayerMoney(ps).."",255,0,0) return end
            local i = doesPlayerAlreadyHaveANumber(exports.server:getPlayerAccountID(ps))
            if type(i) == "number" and i ~= nil and i ~= false then
                exports.NGCdxmsg:createNewDxMessage(ps,"You already have a number ("..i..") in the lottery. The next draw is at 18:00.",255,255,0)
                return
            end
            if num > 0 and num < 31 then
                if isNumberTaken(num) == false then
                    ...
                    exports.NGCdxmsg:createNewDxMessage(ps,"Congratulations! You are now ready for the next draw.",0,255,0)
					exports.NGCdxmsg:createNewDxMessage(ps,"Bought NGC Lotto 720 #"..num.." for $500. Next draw is at 18:00!",0,255,0)
                else
                     exports.NGCdxmsg:createNewDxMessage(ps,""..num.." is already taken, try another number.",255,0,0)
                end
            else
                exports.NGCdxmsg:createNewDxMessage(ps,"The only lottery numbers are 1-30!",255,0,0)
            end
        end
    else
