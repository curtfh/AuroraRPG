...
function setDestinationToPlayer(pl,desc,blip,specificSettings)
	if not(pl) then  exports.NGCdxmsg:createNewDxMessage("GPS: No Player Specified!", 255,0, 0) return end
	if isElement(pl) then
		local name = getPlayerName(pl)
		if getElementData(pl,"isPlayerJailed") == true then
		 exports.NGCdxmsg:createNewDxMessage("GPS: "..name.." is jailed!", 255,0, 0)
		return
		end
		targetPlayer = pl
		...
		setDestination(x,y,z,name,"",specificSettings, targetPlayer)
		 exports.NGCdxmsg:createNewDxMessage("GPS: Target "..name.." in "..getZoneName(x,y,z).."", 0, 255, 0)
...
function destinationReached ()
	...
	if hasDestination == true then
		...
		local message = "You have reached "..locationDesc..""
		 ...
		 exports.NGCdxmsg:createNewDxMessage(message, 0, 255, 0)
...
{
[1]="blips/the.png",
--[[d[1]="blips/customblip_orange.png",
...
--:CSGgps/
	...
	if blip ~= "None" then
		madeBlip = true
		targetBlip = createBlip ( x, y, 25, 59, 3 )
	end
				...
				if isElement(targetPlayer) and getElementType(targetPlayer) == "player" then
					...
					if getElementInterior(targetPlayer) ~= 0 then
						...
						resetDestination()
						 ...
						 exports.NGCdxmsg:createNewDxMessage("GPS: Reset, "..targetName.." is in interior.", 255, 100, 0)
					...
					if isPedDead ( targetPlayer ) then
						...
						resetDestination()
						 ...
						 exports.NGCdxmsg:createNewDxMessage("GPS: Reset, "..targetName.." is dead.", 255, 100, 0)
				 ...
				 exports.NGCdxmsg:createNewDxMessage("GPS: Reset - Target disconnected or is missing!", 255, 0, 0)
					resetDestination()
					return false
				end
...
addEvent("onClientPlayerBackgroundDownload",true)
addEventHandler("onClientPlayerBackgroundDownload",root,function(download)
	if download == "nodes_c.lua" then
		loadstring(exports.CSGmods:getModData(download))()
