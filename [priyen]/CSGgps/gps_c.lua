...
function sayTurn()
	if toSayVoice == true then
		...
		exports.NGCdxmsg:createNewDxMessage("Turn "..markers[source].."",0,255,0)
	end
end
...
function hitLastNode(p)
    if p == localPlayer then
		--endGps()
		-- exports.NGCdxmsg:createNewDxMessage("GPS: Reached Destination "..globalDest.."",255,255,0)
    ...
    if gpsOn == true then
        ...
        if isInPathRange == false then
            ...
            destName = getZoneName(x,y,z)
             exports.NGCdxmsg:createNewDxMessage("GPS: Rerouting / Finding Alternative Route",255,255,0)
			endGps()
            sendGpsReq("",x,y,z)
        end
    end
...
function setGpsC(x,y,z)
    ...
    local pth = calculatePathByCoords(x1,y1,z1,x,y,z)
	drawGps(pth)
	 exports.NGCdxmsg:createNewDxMessage("GPS: Finding Path to "..globalDest.."",255,255,0)
end
