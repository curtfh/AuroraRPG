...
addEventHandler("CSGlaser.batteryDone",root,function()
	exports.NGCdxmsg:createNewDxMessage(source,"Your laser's battery has depleted and the laser have been removed.",0,255,0)
...
addEventHandler("onPlayerLogin",root,function()
	...
	if (type(t) == "boolean") then 
		exports.DENmysql:exec("UPDATE playerstats SET laser=? WHERE userid=?", "[[ ]]",accountID)
		return false 
	end
	t=fromJSON(t)
...
addEventHandler("onPlayerQuit",root,function()
	...
	if oldt then
	oldt = oldt.laser
	oldt=(oldt and fromJSON(oldt) or {0,0,0,0,false})
	local per = oldt[1]
	local r,g,b = GetLaserColor(source)
	local bool = IsLaserEnabled(source)
