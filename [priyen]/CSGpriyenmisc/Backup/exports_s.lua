        ...
        if ( playerStatus ) then
            ...
            if ( wepSkills ) then
                for skillint, valueint in pairs( wepSkills ) do
                    if ( tonumber(valueint) > 950 ) then
                        if tonumber(skillint) == 73 or tonumber(skillint) == 75 then
                        setTimer(function() if isElement(thePlayer) == false then return end setPedStat ( thePlayer, tonumber(skillint), 995 ) end,10000,5)
                        else
                        setTimer(function() if isElement(thePlayer) == false then return end setPedStat ( thePlayer, tonumber(skillint), 1000 ) end,10000,5)
                        end
                    else
                        setTimer(function() if isElement(thePlayer) == false then return end setPedStat ( thePlayer, tonumber(skillint), tonumber(valueint) ) end,10000,5)
                    end
                end
            end
...
addCommandHandler("jrgates",function(ps)
    if exports.server:getPlayerAccountName(ps) == "ralph367" then
...
addEventHandler("onPlayerJoin",root,function()
    ...
    setTimer(function()
        if isElement(p) then
            setCameraMatrix(p, 1482.33, -1722.73, 25.57, 1483.81, -1745.87, 26.19, 0, 178)
        end
    end,1000,1)
end)
