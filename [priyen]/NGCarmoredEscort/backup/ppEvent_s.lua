...
function vehEnter(e,seat)
	...
	if source == truck then
		if isLaw(e) == true then
			...
			--if team == "Military Forces" or team == "GIGN" or team == "Government Agency" then
			--else
			--	 exports.DENdxmsg:createNewDxMessage(e,"Sorry, only GIGN or Military Forces have the authority to drive this truck!",255,255,0)
			--	return
			--end
			if #lawInFromMarker < lawNeeded then
...
addEventHandler("onColShapeHit",root,function(hitElement)
	if source == truckDM then
		if hitElement and getElementType(hitElement) == "player" then
			if not isLaw(hitElement) and getTeamName(getPlayerTeam(hitElement)) ~= "Staff" then
