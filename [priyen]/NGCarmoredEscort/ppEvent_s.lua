...
local truckMaxSpeed = 60 --kmh
local lawNeeded = 2
local timeWaitTillStart = 3 --seconds
local minBags = 2
local maxBags = 2
...
local timeTillEvent = math.random(minTimeBetweenEvents,maxTimeBetweenEvents) -- starts on resource start -- 2 == 1 second after
...
local fromPos = {
	...
	[3] = {-2813.5786132813,375.2692565918,2.8082426071167,99.055331420898,["name"]="SF Whitehouse"},
...
local toPos = {
	...
	[3] = {-2813.5786132813,375.2692565918,3.5082426071167,["name"]="SF Whitehouse"},
	[4] = {165.93356323242,-22.509349822998,0.578125,["name"]="Blueberry Federal Depot"},
	[5] = {1904.693359375,990.34851074219,9.8203125,["name"]="LV The Four Dragons Hotel & Casino"},
	[6] = {2310.3205566406,1647.8489990234,9.8203125,["name"]="LV Caligula's Casino"}
}
local lawTeams = {
	"Military Forces",
	"Government",
...
function preStart()
	...
	truckDM = createColCircle(fromPos[fromI][1],fromPos[fromI][2],35)
	setElementData(truck,"Armored",true)
...
function hitMarker(e)
	if source == fromMarker then
		if getElementType(e) == "player" then
			if exports.DENlaw:isLaw(e) == true then
...
function vehEnter(e,seat)
	...
	if source == truck then
		if exports.DENlaw:isLaw(e) == true then
			...
			if #lawInFromMarker < lawNeeded then
				 exports.NGCdxmsg:createNewDxMessage(e,"Armored Transport Mission: You need "..lawNeeded.." law in the marker to begin!",255,0,0)
			else
				 exports.NGCdxmsg:createNewDxMessage(e,"Armored Transport Mission: Remain in the vehicle! Starting in "..timeWaitTillStart.." seconds!",255,0,0)
...
function start()
	...
	warnCriminals("Armored Truck Mission has been started from Government and has just left "..fromPos[fromI].name.."!")
	...
	truckBlip2 = createBlip(toPos[toI][1],toPos[toI][2],toPos[toI][3],19)
...
function monitorTruck()
	if isElement(truck) then
		...
		if isElement(truckBlip) then
			for k,v in ipairs(getElementsByType("player")) do
				setElementVisibleTo ( truckBlip, v, false )
				if not exports.DENlaw:isLaw(v) then
					setElementVisibleTo ( truckBlip, v, false )
				elseif exports.DENlaw:isLaw(v) then
					setElementVisibleTo ( truckBlip, v, true )
				end
			end
		end
		if isElement(truckBlip2) then
			for k,v in ipairs(getElementsByType("player")) do
				setElementVisibleTo ( truckBlip2, v, false )
				if not exports.DENlaw:isLaw(v) then
					setElementVisibleTo ( truckBlip2, v, false )
				elseif exports.DENlaw:isLaw(v) then
					setElementVisibleTo ( truckBlip2, v, true )
				end
			end
		end
	end
...
function crimWinning()
	...
	warnCriminals("Kill the cops near the truck! You have 2 minutes do the job!")
...
function prepareForMoneybags()
	...
	setTimer(function()
		for k,v in pairs(getElementsWithinColShape(truckCol)) do
			if getElementType(v) == "player" and exports.DENlaw:isLaw(v) then
				exports.NGCdxmsg:createNewDxMessage(v,"Paid $2000 for defending the truck. +2 Score",0,255,0)
...
function vehDamage(loss)
	...
	if source == truck then
		...
		currentHealth=currentHealth-loss*3
	...
	if isElement(truckCol) then
		...
		for k,v in pairs(t) do
			...
			for k2,v2 in pairs(lawTeams) do
				if v2 == exports.DENlaw:isLaw(v) then
					isTypeNear = true
					break
				end
			end
...
function makeMoneyBags(x,y,z)
	...
	for i=1,bagsAm do
		...
		z2=z+0.75
		local m = createMarker(x2,y2,z2,"cylinder",4,255,255,0,0)
...
addEventHandler("onServerPlayerLogin",root,playerLogin)

function hitMoneyBag(e)
	if getElementType(e) == "player" then
		if winning == "law" then if exports.DENlaw:isLaw(e) == false then return end end
		if winning == "crim" then if getTeamName(getPlayerTeam(e)) ~= "Criminals" then return end end
			...
			exports.DENstats:setPlayerAccountData(e,"armoredtrucks",exports.DENstats:getPlayerAccountData(e,"armoredtrucks")+0.5)
			 exports.NGCdxmsg:createNewDxMessage(e,"Picked up $"..bags[i][3]..". Earned +5 Score",0,255,0)
	end
end
...
function tim(ps)
	if timeTillEvent <= 0 then
		exports.NGCdxmsg:createNewDxMessage(ps,"Armored Transport is ready to go",0,255,0)
	else
		if inProgress == true then
			exports.NGCdxmsg:createNewDxMessage(ps,"Armored Transport is currently in progress",0,255,0)
		else
			exports.NGCdxmsg:createNewDxMessage(ps,""..timeTillEvent.." Seconds until Armored Transport is Ready",0,255,0)
		end
	end
end
...
function doEnd()
	...
	local res = getResourceFromName("NGCarmoredEscort")
	restartResource(res)
	end
end
addCommandHandler("resat",function(player,cmd,va)
	if getTeamName(getPlayerTeam(player)) == "Staff" then
		if va == "destroy" then
			doEnd()
		elseif va == "start" then
			if isTimer(timeTillEvent) then killTimer(timeTillEvent) end
			timeTillEvent = 5
		end
	end
end)
addEventHandler("onServerPlayerLogin",root, function () triggerClientEvent(source,"CSGarmoredEventRecTime",source,timeTillEvent) end)
...
addEventHandler("onColShapeHit",root,function(hitElement)
	if source == truckDM then
		if hitElement and getElementType(hitElement) == "player" then
			if not exports.DENlaw:isLaw(hitElement) and getTeamName(getPlayerTeam(hitElement)) ~= "Staff" then
				if getElementData(hitElement,"wantedPoints") < 10 then
					if inProgress == true then
						if isTimer(tim[hitElement]) then return false end
						triggerClientEvent(hitElement,"OutputWarning",root,"Armored Escort","Leave this area within 10 seconds before you get wanted stars.",10)
						tim[hitElement] = setTimer(function(hitElement)
							setElementData(hitElement,"wantedPoints",20)
...
addEventHandler("onColShapeLeave",root,function(hitElement)
	if source == truckDM then
		if hitElement and getElementType(hitElement) == "player" then
			if isTimer(tim[hitElement]) then
				killTimer(tim[hitElement])
				triggerClientEvent(hitElement,"OutputKillWarning",hitElement)
			end
		end
	end
end)
...
addEventHandler("onPlayerQuit",root,function()
	if isTimer(tim[source]) then
		killTimer(tim[source])
	end
	for k,v in pairs(lawInFromMarker) do
		if v == source then
			table.remove(lawInFromMarker,k)
			break
		end
	end
end)
addEventHandler("onPlayerWasted",root,function()
	if isTimer(tim[source]) then
		killTimer(tim[source])
		triggerClientEvent(source,"OutputKillWarning",source)
	end
	for k,v in pairs(lawInFromMarker) do
		if v == source then
			table.remove(lawInFromMarker,k)
...
addEvent("reCol",true)
addEventHandler("reCol",root,function(truck)
	setElementHealth(truck,1000)
	outputDebugString("Fixing after flash")
end)
