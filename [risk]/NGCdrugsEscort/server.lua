...
local RewardMAX = 120 --Max reward
local RewardMIN = 80 --Min reward
local crimNeeded = 2
...
{692.83, 1944.87, 5.53, 176, "LV Safe house"},
}
----------------------------------------------------
...
function canCrimEnter ( cr )
	local state = true
	for k, dr in ipairs ( joined ) do
		if ( dr == exports.server:getPlayerAccountName( cr ) ) then
			state = false
		end
	end
	return state
...
function DrugDeliveryVan()
	...
	setElementData(theVan,"ArmoredDT",true)
	...
	if txt1 == txt2 then
		local x2, y2, z2, r2, txt2 = unpack ( VanDropLocations [ math.random ( #VanDropLocations ) ] )
			...
			for k,v in ipairs(getElementsByType("player")) do
				if isCrim(v) then
					exports.NGCdxmsg:createNewDxMessage(v,"[Drugs delivery Mission] : "..txt1.." is requesting to deliver drugs package to "..txt2.."!",0,255,0)
					exports.NGCdxmsg:createNewDxMessage(v,"[Drugs delivery Mission] : You can search for 'Truck' blip in map (F11)!",0,255,0)
...
function hitManage(hitElement)
	if hitElement and getElementType(hitElement) == "player" and not isPedInVehicle(hitElement) and isPedOnGround(hitElement) then
		if isCrim(hitElement) then
			if isTimer ( TimeEventLeft) then return end
			table.insert(crimsInFromMarker,hitElement)
			checkEvent(hitElement)
		end
	end
end
...
function DeliveryStatus(thePlayer)
	if isCrim(thePlayer) or exports.CSGstaff:isPlayerStaff(thePlayer) then
		if DeliveryTruckStatus == true then
			if isTimer(TimeEventLeft) then
				...
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] :  Event is already in proggress!",255,0,0)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] : Truck has left "..FromWhere.." and is heading to "..ToWhere.."!",255,0,0)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,""..math.ceil(remaining/60000).." mins left untill event will be finished!",255,0,0)
			elseif isTimer(TimeToCollectReward) then
				remaining, executesRemaining, totalExecutes = getTimerDetails(TimeToCollectReward)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] :  Truck Event has successfully finished!",255,0,0)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] : Pick up reward fast!",255,0,0)
				exports.AURcriminalp:giveCriminalPoints(thePlayer, "BankATMRob", 3)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,""..math.ceil(remaining/1000).." second(s) left untill you're able to pick up reward!",255,0,0)
			elseif isTimer(TimeLeftUntilCanceled) then
				remaining, executesRemaining, totalExecutes = getTimerDetails(TimeLeftUntilCanceled)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] : Drugs Truck is waiting for delivery",255,0,0)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] : "..FromWhere.." is requesting to transport drugs package to "..ToWhere.."!",255,0,0)
				exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] : "..math.ceil(remaining/60000).." minute(s) left untill event will get canceled!",255,0,0)
			end
		else
			remaining, executesRemaining, totalExecutes = getTimerDetails(eventTimeRoot)
			exports.NGCdxmsg:createNewDxMessage(thePlayer,"[Drugs delivery Mission] :  Event has finished or canceled!",255,0,0)
			outputChatBox("[DT] Next event will be ready in next "..math.ceil(remaining/60000).." minute(s)!",thePlayer,0,255,0)
		end
	end
end
...
function enterVehicle ( thePlayer, seat, jacked )
    if ( source == theVan) and (not isCrim(thePlayer)) and (seat == 0) then
		cancelEvent()
        exports.NGCdxmsg:createNewDxMessage(thePlayer,"Only criminals are allowed to drive this vehicle!",255,0,0)
    end
end
...
function enterVehicle ( vehicle, seat, jacked )
    if ( vehicle == theVan ) then
		if isCrim(source) then
			if seat == 0 then
				if isCrim(source) then
					...
					if (#crimsInFromMarker < crimNeeded) then
						exports.NGCdxmsg:createNewDxMessage(source,"[Drugs delivery Mission] : You need "..crimNeeded.." criminals in the marker to begin!",255,0,0)
					else
						for k, v in ipairs(getElementsByType("player")) do
							if isCrim(v) then
								exports.NGCdxmsg:createNewDxMessage(v,"[Drugs delivery Mission] : Truck just left "..FromWhere.." and is heading to "..ToWhere.." (Red dragon blip)!",0,255,0)
			...
			exports.NGCdxmsg:createNewDxMessage(source,"[Drugs delivery Mission] : You are not allowed to enter in this vehicle!",255,0,0)
...
function checkEvent(source)
	if isTimer ( TimeEventLeft) then return end

	if (#crimsInFromMarker < crimNeeded) then
		exports.NGCdxmsg:createNewDxMessage(source,"[Drugs delivery Mission] : You need "..tostring(crimNeeded - #crimsInFromMarker).." more criminal(s) in the marker to begin!",255,0,0)
	else
		for k, v in ipairs(getElementsByType("player")) do
			if isCrim(v) then
				exports.NGCdxmsg:createNewDxMessage(v,"[Drugs delivery Mission] : Truck just left "..FromWhere.." and is heading to "..ToWhere.." (Red dragon blip)!",0,255,0)
		...
		setVehicleEngineState(theVan,true)
		setVehicleDamageProof(theVan,false)
		if isTimer ( TimeLeftUntilCanceled ) then 
			killTimer ( TimeLeftUntilCanceled ) 
		end
	end
end
...
function onHitMarker(element)
	if element == theVan then
		...
		for k, v in ipairs(getElementsByType("player")) do
			if isCrim(v) then
				exports.NGCdxmsg:createNewDxMessage(v,"[Drugs delivery Mission] : Get the drugs within 1 min!",255,0,0)
			end
		end
...
if( getElementType( hitElement ) ~= "player" ) then return end
	...
	if canCrimEnter(hitElement) then
		if isCrim(hitElement) then
		...
		local mon = math.random(5000,7500)
		givePlayerMoney(hitElement,mon)
		exports.CSGgroups:addXP(hitElement,9)
		exports.NGCdxmsg:createNewDxMessage(hitElement,"[Drugs delivery Mission] : Truck successfully transported to "..ToWhere.."",50,250,80)
		exports.NGCdxmsg:createNewDxMessage(hitElement,"Picked up "..reward1.." hits of Ritalin & "..reward2.." hits of Weed.",50,250,80)
		exports.NGCdxmsg:createNewDxMessage(hitElement,"Picked up $"..mon..".",50,250,80)
		setElementData(hitElement,"ArmoredTruck",false)
		table.insert( joined, exports.server:getPlayerAccountName(hitElement) )
		end
	end
end
...
function destroyDeliveryVan()
	if (isElement(theVan) and isElement(theVanBlip) and isElement(theFinishBlip) and isElement(theFinishMarker)) then
		for k,v in ipairs(getElementsByType("player")) do
			if isCrim(v) then
				exports.NGCdxmsg:createNewDxMessage(v,"[Drugs delivery Mission] : Event has been finished/destroyed! *",0,255,0)
				setElementData(v,"ArmoredTruck",false)
			end
		end
