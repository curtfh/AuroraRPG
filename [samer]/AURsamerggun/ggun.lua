...
function toggleGGun(player)
	if ((type(exports.csgstaff:getPlayerAdminLevel(player)) == "number") and (exports.csgstaff:getPlayerAdminLevel(player) >= 5)) then
	local on = not isGravityGunEnabled(player)
	togglePlayerGravityGun(player,on)
	exports.NGCdxmsg:createNewDxMessage("Gravity gun "..(on and "on" or "off"),player,255,255,255)
	end
end
