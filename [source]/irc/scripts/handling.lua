	...
	function (socket)
		for server,info in pairs (servers) do
			if info[1] == socket then
				...
				setTimer(ircRaw,5000,1,server,"OPER ngcbot daputamadre")
				setTimer(ircRaw,8000,1,server,"MODE #echo +q ECHOBot")
				setTimer(ircRaw,8000,1,server,"MODE #staff +q ECHOBot")
                                      --setTimer(outputIRC,15000,1,"!restart CSGstaff")
				return
			end
		end
	end
...
addEventHandler("onIRCRaw",root,
	function (data)
		...
		if t[2] == "NICK" then
			...
			if(newnick:sub(0,1) == ":") then newnick = newnick:sub(2) end
