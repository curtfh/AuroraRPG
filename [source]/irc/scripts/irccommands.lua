...
addEventHandler("onIRCResourceStart",root,
	function ()
		...
		addIRCCommandHandler("!pm",
			function (server,channel,user,command,name,...)
				...
				if player then
					...
					exports.CSGlogging:createLogRow ( player, "IRCPM", "<- "..ircGetUserNick(user)..": "..message )
				else
		...
		addIRCCommandHandler("!kick",
			function (server,channel,user,command,name,...)
				...
				if player then
					...
					outputChatBox("(IRC) "..nick.." was kicked from the game by "..ircGetUserNick(user).." ("..reason..")",root,255,0,0)
				else
					ircNotice(user,"'"..name.."' no such player")
				end
			end
		...
		addIRCCommandHandler("!unmute",
			function (server,channel,user,command,name,...)
				...
				if player then
					...
					outputChatBox("(IRC) "..getPlayerName(player).." was unmuted by "..ircGetUserNick(user),root,0,255,0)
				else
					ircNotice(user,"'"..name.."' no such player")
				end
			end
		...
		addIRCCommandHandler("!unfreeze",
			function (server,channel,user,command,name)
				...
				if player then
					...
					outputChatBox("(IRC) "..getPlayerName(player).." was unfrozen by "..ircGetUserNick(user),root,0,255,0)
		...
		addIRCCommandHandler("!banserial",
			function (server,channel,user,command,serial,time, ...)
				if(channel == nil) then return end
				if not serial or not tonumber(time) then ircNotice(user,"syntax is !banserial <serial> <time> (<reason>)") return end
				local reason = table.concat({...}," ") or ""
				addBan(nil,nil,serial,root,"(by "..ircGetUserNick(user)..") "..reason,tonumber(time))
			end
		)
		...
		addIRCCommandHandler("!lua",
			function (server,channel,user,command,...)
				...
				runString(str,root,ircGetUserNick(user), channel)
			end
		)
		...
		addIRCCommandHandler("!crun",
			function (server,channel,user,command,...)
				...
				if player then
					table.remove(t,1)
					str = table.concat(t," ")
					triggerClientEvent(player,"irc_doCrun",resourceRoot,str,true)
				else
					...
					for i,player in ipairs (getElementsByType("player")) do
						if i == 1 then
							...
							triggerClientEvent(player,"irc_doCrun",resourceRoot,str,false)
						end
					end
				end
			end
...
--[[
		...
		addIRCCommandHandler("!country",
			function (server,channel,user,command,name)
				...
				local country = getResourceFromName( "Country" )
				if country and getResourceState( country ) == "running" then
					local player = getPlayerFromPartialName( name )
					if player then
					...
					ircNotice(user, "the country resource is not running")
				end
			end
		)
