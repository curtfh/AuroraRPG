...
addEventHandler("onIRCResourceStart",root,
	function ()
		...
		addIRCCommandHandler("!supporters",
			function (server,channel,user,command,arg)
				local supporters = {}
				for _,player in ipairs(getElementsByType("player")) do
					if getElementData(player,"isPlayerSupporter") then
						table.insert(supporters,getPlayerName(player))
					end
				end
				if(#supporters > 0) then
					ircSay(channel,"Online supporters: "..table.concat(supporters,", "))
				else
					ircSay(channel,"No supporters currently online!")
				end
			end
		)
	end
