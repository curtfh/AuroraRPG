...
addEventHandler("onResourceStart",resourceRoot,
	function ()
		...
		if not sockOpen then
			outputDebugString("IRC: could not start resource, the sockets module isn't loaded!")
			outputDebugString("IRC: restart the resource to retry")
			return
		end
		...
		if #missingRights ~= 0 then
			...
			outputDebugString("IRC: could not start resource, the resource is missing rights!")
			outputDebugString("IRC: restart the resource to retry")
			return
		end
		...
		function checkVersion (res,version)
			if res ~= "ERROR" and version then
				if getNumberFromVersion(version) > getNumberFromVersion(getResourceInfo(getThisResource(),"version")) then
					outputDebugString("IRC: resource is outdated, newest version: "..version)
		...
		if  aclFile then
			...
			while(xmlFindChild(aclFile,"command",i)) do
				...
				if commands[string.lower(attrs.name)] then
					...
					commands[string.lower(attrs.name)].echoChannelOnly = attrs.echoChannelOnly == "true"
				else
					commands[string.lower(attrs.name)] = {level = tonumber(attrs.level),echoChannelOnly = attrs.echoChannelOnly == "true"}
				end
				i = i + 1
			end
			xmlUnloadFile(aclFile)
		else
			outputDebugString("IRC: could not start resource, the acl file can't be loaded!")
			outputDebugString("IRC: restart the resource to retry")
			return
		end
...
function internalConnect ()
	...
	if settingsFile then
		for i,server in ipairs (xmlNodeGetChildren(settingsFile)) do
			...
			if not host then
				outputDebugString("IRC: problem with server #"..i..", no host given!")
			elseif not nick then
				outputDebugString("IRC: problem with server #"..i..", no nick given!")
			elseif not channels then
				outputDebugString("IRC: problem with server #"..i..", no channels given!")
			else
				...
				if server then
					if nspass then
						...
						outputDebugString("Identify")
					end
					for i,channel in ipairs (split(channels,44)) do
						ircJoin(server,gettok(channel,1,32),gettok(channel,2,32))
					end
				else
					outputDebugString("IRC: problem with server #"..i..", could not connect ("..tostring(getSocketErrorString(sockError)..")"))
				end
			end
		end
		xmlUnloadFile(settingsFile)
	else
		outputDebugString("IRC: could not start resource, the settings.xml can't be parsed!")
		outputDebugString("IRC: restart the resource to retry")
		return
	end
end
...
function registerFunction (name,ft,...)
	...
	for i,arg in ipairs (arguments) do
		if string.sub(arg,1,1) == "(" and string.sub(arg,-1) == ")" then
			arguments[i] = "if args["..i.."] and type(args["..i.."]) ~= '"..string.sub(arg,2,-2).."' then return not outputDebugString(prefix..' bad argument #"..i.." at "..name.." "..arg.." expected, got '..type(args["..i.."])) end"
		elseif string.find(arg,"/",0,true) then
			...
			arguments[i] = "if "..str.." then return not outputDebugString(prefix..' bad argument #"..i.." at "..name.." "..arg.." expected, got '..type(args["..i.."])) end"
		else
			arguments[i] = "if type(args["..i.."]) ~= '"..arg.."' then return not outputDebugString(prefix..' bad argument #"..i.." at "..name.." "..arg.." expected, got '..type(args["..i.."])) end"
		end
	end
	r, err = loadstring("function "..name.." (...) local args = {...} local sourcename = type(sourceResource) == 'userdata' and getResourceName(sourceResource) or 'irc' local prefix = '[irc] '..sourcename "..table.concat(arguments," ").." return "..ft.."(...) end")()
	--_G[name] = _G[ft]
end
