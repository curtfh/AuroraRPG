...
setTimer(function()
	for i,channel in pairs (ircGetChannels()) do
		if ircIsEchoChannel(channel) then
			ircJoin(ircGetChannelServer(channel),"EchoBot","darkpro123")
		end
	end
end,5000,1)]]
...
function func_ircSay (target,message)
	...
	elseif type(target) == "irc-staff" then
		triggerEvent("onIRCPrivateMessage",target,message)
		return ircRaw(server,"PRIVMSG #staff :"..message)
	end
	return false
end
...
function func_outputIRCStaff (message)
	if #message > 400 then
		for i=1,math.ceil(#message/400) do
			outputIRC(string.sub(message,(i-1)*400,i*400))
		end
	...
	for i,channel in pairs (ircGetChannels()) do
		if not ircIsEchoChannel(channel) then
			local server = getElementParent(channel)
			local localuser = ircGetUserFromNick(ircGetServerNick(server))
			if server then
				if localuser then
...
registerFunction("outputIRCStaff","func_outputIRCStaff","string")
