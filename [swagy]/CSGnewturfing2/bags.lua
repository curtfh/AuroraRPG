...
function msg(m,r,g,b)
	if getElementData(localPlayer,"turfBag") then
		exports.NGCdxmsg:createNewDxMessage(m,r,g,b)
	end
end

addEvent("tsRecBagInfo",true)
addEventHandler("tsRecBagInfo",localPlayer,function(x,t,hideMsg)
	if (not turfSpawnTable[x]) then
		return false
	end
	bags[x] = t
		...
		for i,v in pairs(bags) do
			...
			if turfSpawnTable[i] then
				...
				if gr == v[2] and not(hideMsg) and (getTeamName(getPlayerTeam(localPlayer)) == "Criminals") then
					msg("A turf money bag is available for pickup in "..getZoneName(x,y,z).."",0,255,0)
				else
		...
		if not isPedInVehicle(p) then
			...
			if getTeamName(getPlayerTeam(localPlayer)) == "Criminals" then
				if (gr) then
					...
					msg("You must be in a group to steal this turf money bag",255,0,0)
				end
			else
				--triggerEvent("tsKillBag",localPlayer,i)
			end
